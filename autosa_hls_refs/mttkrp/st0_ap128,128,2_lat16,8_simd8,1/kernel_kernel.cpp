// AutoSA Command:
// /workspaces/mlir-systolic/third_party/AutoSA/autosa /workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/mttkrp/kernel.c --config=/workspaces/mlir-systolic/third_party/AutoSA/autosa_config/autosa_config.json --target=autosa_hls_c --output-dir=/workspaces/mlir-systolic/third_party/AutoSA/autosa.tmp/output --host-serialize --simd-info=/workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/mttkrp/simd_info.json --sa-sizes={kernel[]->space_time[0];kernel[]->array_part[128,128,2];kernel[]->latency[16,8];kernel[]->simd[8,1]}
...
void A_IO_L1_in_intra_trans(int idx, int c0, int c1, int c2, A_t8 local_A[16][2][32], hls::stream<A_t8> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L1_in_inter_trans(int idx, int c0, int c1, int c2, A_t8 local_A[16][2][32], hls::stream<A_t8> &fifo_A_in, hls::stream<A_t8> &fifo_A_out, bool inter_trans_en);
void A_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c2, A_t8 local_A[16][2][32], hls::stream<A_t8> &fifo_A_in, bool inter_trans_en);
...
void B_IO_L2_in_intra_trans(int c0, int c1, int c2, B_t16 local_B[2][8], hls::stream<float> &fifo_B_local_out, bool intra_trans_en);
void B_IO_L2_in_inter_trans_boundary(int c0, int c1, int c2, B_t16 local_B[2][8], hls::stream<B_t16> &fifo_B_in, bool inter_trans_en);
void C_IO_L2_in_intra_trans(int c0, int c1, int c2, C_t16 local_C[128][16], hls::stream<C_t8> &fifo_C_local_out, bool intra_trans_en);
void C_IO_L2_in_inter_trans_boundary(int c0, int c1, C_t16 local_C[128][16], hls::stream<C_t16> &fifo_C_in, bool inter_trans_en);
...
void D_drain_IO_L1_out_intra_trans(int idx, int c0, int c1, D_t4 local_D[16][32], hls::stream<float> &fifo_D_drain_local_in);
void D_drain_IO_L1_out_inter_trans(int idx, int c0, int c1, D_t4 local_D[16][32], hls::stream<D_t4> &fifo_D_drain_in, hls::stream<D_t4> &fifo_D_drain_out);
void D_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c1, D_t4 local_D[16][32], hls::stream<D_t4> &fifo_D_drain_out);
...
#pragma HLS INLINE OFF
  ...
  // io_L1
  // pe
  for (int c4 = 0; c4 <= min(15, -16 * c1 + 41); c4 += 1)
    for (ap_uint<2> c5 = 0; c5 <= 1; c5 += 1)
      for (ap_uint<6> c6 = 0; c6 <= 31; c6 += 1) {
        // latency
        ...
        for (ap_uint<4> c7 = 0; c7 <= 7; c7 += 1) {
          ...
          #pragma HLS PIPELINE II=1
            // simd
            {
              B_t16 in_data;
              B_t1 out_data;
              in_data = local_B[c5][(8 * c4 + c7) / 16];
              for (ap_uint<5> n = 0; n < 16; n++) {
              #pragma HLS UNROLL
                data_split[n] = in_data(31, 0);
                in_data = in_data >> 32;
              }
              int split_idx = (8*c4 + c7) % 16;
...
#pragma HLS INLINE OFF
  ...
  for (ap_uint<2> c3 = 0; c3 <= 1; c3 += 1) {
    // access_coalesce
    // access_serialize
    for (int c4 = 0; c4 <= min(7, -8 * c1 + 20); c4 += 1) {
    #pragma HLS PIPELINE II=1
      {
        B_t16 in_data;
        B_t16 out_data;
        ...
        for (ap_uint<4> c7 = 0; c7 <= 7; c7 += 1) {
          ...
          #pragma HLS PIPELINE II=1
            // simd
            {
              C_t16 in_data;
              C_t8 out_data;
              in_data = local_C[8*c4 + c7][8 * c6 / 16];
...
#pragma HLS INLINE OFF
  ...
  // io_L2
  for (int c3 = 0; c3 <= min(127, -128 * c1 + 335); c3 += 1) {
    // access_coalesce
    // access_serialize
    for (ap_uint<5> c4 = 0; c4 <= 15; c4 += 1) {
    #pragma HLS PIPELINE II=1
                ...
                #pragma HLS PIPELINE II=1
                  {
                    ...
                    #pragma HLS UNROLL
                      local_D[c8][8*c4 + c7] = (local_D[c8][8*c4 + c7] + ((local_A[0][0][c9] * local_B[0][0]) * local_C[0][c9]));
                    }
                    if (c2 == 127 && c5 == 1 && c6 == 31)
                      fifo_D_drain_out.write(local_D[c8][8*c4 + c7]);
                    {
                      C_t8 fifo_data;
    ...
    for (ap_uint<4> c7 = 0; c7 <= 7; c7 += 1) {
      ...
      #pragma HLS PIPELINE II=1
        // simd
        {
          ...
          int split_idx = (8*c4 + c7) % 4;
          out_data = local_D[c8][(8 * c4 + c7) / 4];
          ...
          out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);          local_D[c8][(8 * c4 + c7) / 4] = out_data;
        }
      }
    }
  }
...
#pragma HLS INLINE
  ...
  for (ap_uint<4> c3 = p0; c3 <= 7; c3 += 1) {
    ...
    if (c3 == p0) {
      for (ap_uint<5> c4 = 0; c4 <= 15; c4 += 1) {
        // access_coalesce
        for (int c5 = 0; c5 <= min(31, -32 * c1 + 83); c5 += 1) {
        #pragma HLS PIPELINE II=1
          {
            D_t4 in_data;
            D_t4 out_data;
