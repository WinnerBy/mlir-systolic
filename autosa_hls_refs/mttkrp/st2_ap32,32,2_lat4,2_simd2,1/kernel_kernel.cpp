// AutoSA Command:
// /workspaces/mlir-systolic/third_party/AutoSA/autosa /workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/mttkrp/kernel.c --config=/workspaces/mlir-systolic/third_party/AutoSA/autosa_config/autosa_config.json --target=autosa_hls_c --output-dir=/workspaces/mlir-systolic/third_party/AutoSA/autosa.tmp/output --host-serialize --simd-info=/workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/mttkrp/simd_info.json --sa-sizes={kernel[]->space_time[2];kernel[]->array_part[32,32,2];kernel[]->latency[4,2];kernel[]->simd[2,1]}
...
void A_IO_L1_in_intra_trans(int idx, int c0, int c1, int c2, A_t4 local_A[32][1][64], hls::stream<A_t2> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L1_in_inter_trans(int idx, int c0, int c1, int c2, A_t4 local_A[32][1][64], hls::stream<A_t4> &fifo_A_in, hls::stream<A_t4> &fifo_A_out, bool inter_trans_en);
void A_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c2, A_t4 local_A[32][1][64], hls::stream<A_t4> &fifo_A_in, bool inter_trans_en);
...
void C_IO_L1_in_intra_trans(int idx, int c0, int c1, C_t4 local_C[2][64], hls::stream<C_t2> &fifo_C_local_out, bool intra_trans_en);
...
void D_1_IO_L2_in_intra_trans(int c0, int c1, int c2, D_t2 *D, D_t2 local_D_1[32][1], hls::stream<float> &fifo_D_1_local_out, bool intra_trans_en);
...
void D_1_IO_L2_out_intra_trans(int c0, int c1, int c2, D_t2 *D, D_t2 local_D_1[32][1], hls::stream<float> &fifo_D_1_local_in, bool intra_trans_en);
void D_1_IO_L2_out_inter_trans_boundary(int c0, int c1, D_t2 *D, D_t2 local_D_1[32][1], hls::stream<D_t2> &fifo_D_1_out, bool inter_trans_en);
void D_drain_IO_L1_out_intra_trans(int idx, int c0, int c1, D_t2 local_D[32][1], hls::stream<float> &fifo_D_drain_local_in);
void D_drain_IO_L1_out_inter_trans(int idx, int c0, int c1, D_t2 local_D[32][1], hls::stream<D_t2> &fifo_D_drain_in, hls::stream<D_t2> &fifo_D_drain_out);
void D_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c1, D_t2 local_D[32][1], hls::stream<D_t2> &fifo_D_drain_out);
            ...
            #pragma HLS PIPELINE II=1
              {
                ...
                A_t4 out_data;
                if (c6 % 4 == 0) {
                  in_data = A[(((32 * c0 + c4) * 256 + (32 * c2 + c3)) * 256 + 4 * c6) / 16];
                ...
                int split_idx = (524288*c0 + 2048*c2 + 64*c3 + 16384*c4 + c6) % 4;
      ...
      for (ap_uint<2> c7 = 0; c7 <= 1; c7 += 1) {
        ...
        #pragma HLS PIPELINE II=1
          // simd
          {
            A_t4 in_data;
            A_t2 out_data;
            in_data = local_A[4*c4 + c8][0][2 * c6 / 4];
...
#pragma HLS INLINE OFF
  ...
  for (ap_uint<6> c3 = p0; c3 <= 31; c3 += 1) {
    // io_L1
    if (c3 == p0) {
      for (ap_uint<6> c4 = 0; c4 <= 31; c4 += 1) {
        // access_coalesce
    ...
    for (ap_uint<2> c7 = 0; c7 <= 1; c7 += 1) {
      ...
      #pragma HLS PIPELINE II=1
        // simd
        {
          D_t2 in_data;
          D_t1 out_data;
          in_data = local_D_1[4*c4 + c8][c7 / 2];
          for (ap_uint<2> n = 0; n < 2; n++) {
          #pragma HLS UNROLL
            data_split[n] = in_data(31, 0);
            in_data = in_data >> 32;
              ...
              #pragma HLS PIPELINE II=1
                {
                  ...
                  if (p0 + 32 * c2 >= 1 && c6 == 0)
                    local_D[4*c4 + c8][c7] = fifo_D_1_in.read();
                  // simd
                  for (ap_uint<2> c9 = 0; c9 <= 1; c9 += 1) {
                  #pragma HLS UNROLL
                    local_D[4*c4 + c8][c7] = (local_D[4*c4 + c8][c7] + ((local_A[0][0][c9] * local_B[0][0]) * local_C[0][c9]));
                  }
                  if (p0 == 31 && c2 == 7 && c6 == 127) {
                    fifo_D_drain_out.write(local_D[4*c4 + c8][c7]);
                  } else if (p0 + 32 * c2 <= 254 && c6 == 127) {
                    fifo_D_1_out.write(local_D[4*c4 + c8][c7]);
                  }
                }
              }
            }
    ...
    for (ap_uint<2> c7 = 0; c7 <= 1; c7 += 1) {
      ...
      #pragma HLS PIPELINE II=1
        // simd
        {
          ...
          int split_idx = (c7) % 2;
          out_data = local_D_1[4*c4 + c8][c7 / 2];
          for (ap_uint<2> n = 0; n < 2; n++) {
          #pragma HLS UNROLL
          ...
          out_data = (data_split[1], data_split[0]);          local_D_1[4*c4 + c8][c7 / 2] = out_data;
        }
      }
    }
  }
    ...
    for (ap_uint<2> c7 = 0; c7 <= 1; c7 += 1) {
      ...
      #pragma HLS PIPELINE II=1
        // simd
        {
          ...
          u.ut = in_data;
          data_split[split_idx] = ap_uint<32>(u.ui);
          out_data = (data_split[1], data_split[0]);          local_D[4*c4 + c8][c7 / 2] = out_data;
        }
      }
    }
  }
