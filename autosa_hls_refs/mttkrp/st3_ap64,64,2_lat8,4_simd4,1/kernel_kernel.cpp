...
void A_IO_L2_in_intra_trans(int idx, int c0, int c1, int c2, A_t16 local_A[8][2][16], hls::stream<A_t4> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L2_in_inter_trans(int idx, int c0, int c1, int c2, A_t16 local_A[8][2][16], hls::stream<A_t16> &fifo_A_in, hls::stream<A_t16> &fifo_A_out, bool inter_trans_en);
void A_IO_L2_in_inter_trans_boundary(int idx, int c0, int c1, int c2, A_t16 local_A[8][2][16], hls::stream<A_t16> &fifo_A_in, bool inter_trans_en);
void B_IO_L2_in_intra_trans(int idx, int c0, int c1, int c2, B_t4 local_B[2][1], hls::stream<float> &fifo_B_local_out, bool intra_trans_en);
...
void C_IO_L2_in_intra_trans(int idx, int c0, int c1, C_t16 local_C[4][16], hls::stream<C_t4> &fifo_C_local_out, bool intra_trans_en);
void C_IO_L2_in_inter_trans(int idx, int c0, int c1, C_t16 local_C[4][16], hls::stream<C_t16> &fifo_C_in, hls::stream<C_t16> &fifo_C_out, bool inter_trans_en);
void C_IO_L2_in_inter_trans_boundary(int idx, int c0, int c1, C_t16 local_C[4][16], hls::stream<C_t16> &fifo_C_in, bool inter_trans_en);
...
void D_drain_IO_L1_out_intra_trans(int idx, int idy, int c0, int c1, D_t4 local_D[8][1], hls::stream<float> &fifo_D_drain_local_in);
void D_drain_IO_L1_out_inter_trans(int idx, int idy, int c0, int c1, D_t4 local_D[8][1], hls::stream<D_t4> &fifo_D_drain_in, hls::stream<D_t4> &fifo_D_drain_out);
void D_drain_IO_L1_out_inter_trans_boundary(int idx, int idy, int c0, int c1, D_t4 local_D[8][1], hls::stream<D_t4> &fifo_D_drain_out);
...
#pragma HLS INLINE OFF
  ...
  for (ap_uint<24> i = 0; i < 6291456; i++) {
  #pragma HLS PIPELINE II=1
    A_t16 fifo_data;
    fifo_data = A[i];
  ...
  for (ap_uint<3> c0 = 0; c0 <= 3; c0 += 1)
    for (ap_uint<4> c1 = 0; c1 <= 5; c1 += 1)
      for (ap_uint<8> c2 = 0; c2 <= 127; c2 += 1) {
        // array
        // io_L3
        for (int c3 = 0; c3 <= min(15, -16 * c1 + 83); c3 += 1) {
          // io_L2
...
#pragma HLS INLINE OFF
  ...
  for (int c3 = p0; c3 <= min(15, -16 * c1 + 83); c3 += 1) {
    // io_L2
    if (c3 == p0) {
      for (ap_uint<2> c4 = 0; c4 <= 1; c4 += 1) {
      #pragma HLS PIPELINE II=1
...
#pragma HLS INLINE OFF
  ...
  for (ap_uint<23> i = 0; i < 2752512; i++) {
  #pragma HLS PIPELINE II=1
    C_t16 fifo_data;
    fifo_data = C[i];
