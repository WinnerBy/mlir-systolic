// AutoSA Command:
// /workspaces/mlir-systolic/third_party/AutoSA/autosa /workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/mttkrp/kernel.c --config=/workspaces/mlir-systolic/third_party/AutoSA/autosa_config/autosa_config.json --target=autosa_hls_c --output-dir=/workspaces/mlir-systolic/third_party/AutoSA/autosa.tmp/output --host-serialize --simd-info=/workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/mttkrp/simd_info.json --sa-sizes={kernel[]->space_time[1];kernel[]->array_part[256,256,4];kernel[]->latency[32,16];kernel[]->simd[16,2]}
...
void A_IO_L2_in_intra_trans(int c1, int c2, A_t16 local_A[256][4][16], hls::stream<A_t16> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L2_in_inter_trans_boundary(int c1, int c2, A_t16 local_A[256][4][16], hls::stream<A_t16> &fifo_A_in, bool inter_trans_en);
void B_IO_L1_in_intra_trans(int idx, int c1, int c2, B_t4 local_B[4][8], hls::stream<float> &fifo_B_local_out, bool intra_trans_en);
void B_IO_L1_in_inter_trans(int idx, int c1, int c2, B_t4 local_B[4][8], hls::stream<B_t4> &fifo_B_in, hls::stream<B_t4> &fifo_B_out, bool inter_trans_en);
void B_IO_L1_in_inter_trans_boundary(int idx, int c1, int c2, B_t4 local_B[4][8], hls::stream<B_t4> &fifo_B_in, bool inter_trans_en);
...
void C_IO_L1_in_intra_trans(int idx, int c1, C_t16 local_C[32][16], hls::stream<C_t16> &fifo_C_local_out, bool intra_trans_en);
void C_IO_L1_in_inter_trans(int idx, int c1, C_t16 local_C[32][16], hls::stream<C_t16> &fifo_C_in, hls::stream<C_t16> &fifo_C_out, bool inter_trans_en);
void C_IO_L1_in_inter_trans_boundary(int idx, int c1, C_t16 local_C[32][16], hls::stream<C_t16> &fifo_C_in, bool inter_trans_en);
void C_IO_L1_in_wrapper(int idx, hls::stream<C_t16> &fifo_C_in, hls::stream<C_t16> &fifo_C_out, hls::stream<C_t16> &fifo_C_local_out);
void C_IO_L1_in_boundary_wrapper(int idx, hls::stream<C_t16> &fifo_C_in, hls::stream<C_t16> &fifo_C_local_out);
void PE_wrapper(int idx, hls::stream<A_t16> &fifo_A_in, hls::stream<A_t16> &fifo_A_out, hls::stream<float> &fifo_B_in, hls::stream<C_t16> &fifo_C_in, hls::stream<float> &fifo_D_drain_out);
void D_drain_IO_L1_out_intra_trans(int idx, int c1, D_t4 local_D[256][8], hls::stream<float> &fifo_D_drain_local_in);
void D_drain_IO_L1_out_inter_trans(int idx, int c1, D_t4 local_D[256][8], hls::stream<D_t4> &fifo_D_drain_in, hls::stream<D_t4> &fifo_D_drain_out);
void D_drain_IO_L1_out_inter_trans_boundary(int idx, int c1, D_t4 local_D[256][8], hls::stream<D_t4> &fifo_D_drain_out);
        ...
        for (ap_uint<5> c7 = 0; c7 <= 15; c7 += 1) {
          ...
          #pragma HLS PIPELINE II=1
            // simd
            {
              A_t16 in_data;
              A_t16 out_data;
              in_data = local_A[16*c4 + c7][c5][c6];
              out_data = in_data;
...
/* Module Definition */
void A_IO_L2_in_boundary(hls::stream<A_t16> &fifo_A_in, hls::stream<A_t16> &fifo_A_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  A_t16 local_A_ping[256][4][16];
  #pragma HLS RESOURCE variable=local_A_ping core=RAM_1P_BRAM
  A_t16 local_A_pong[256][4][16];
...
#pragma HLS INLINE OFF
  ...
  for (ap_uint<2> c1 = 0; c1 <= 1; c1 += 1)
    for (ap_uint<7> c2 = 0; c2 <= 63; c2 += 1) {
      // array
      // io_L2
      for (int c3 = 0; c3 <= min(7, -8 * c1 + 10); c3 += 1) {
        // io_L1
        for (ap_uint<3> c4 = 0; c4 <= 3; c4 += 1) {
          // access_coalesce
          // access_serialize
          for (int c5 = 0; c5 <= min(7, -64 * c1 - 8 * c3 + 83); c5 += 1) {
          #pragma HLS PIPELINE II=1
            {
              B_t4 in_data;
              B_t4 out_data;
  ...
  for (ap_uint<5> c4 = 0; c4 <= 15; c4 += 1)
    for (ap_uint<3> c5 = 0; c5 <= 3; c5 += 1)
      for (ap_uint<5> c6 = 0; c6 <= 15; c6 += 1) {
        ...
        for (ap_uint<5> c7 = 0; c7 <= 15; c7 += 1) {
          // latency
          for (int c8 = 0; c8 <= min(31, -32 * p0 - 256 * c1 + 335); c8 += 1) {
          #pragma HLS PIPELINE II=1
            // simd
            {
              B_t4 in_data;
...
#pragma HLS INLINE OFF
  ...
  for (int c3 = p0; c3 <= min(7, -8 * c1 + 10); c3 += 1) {
    // io_L1
    if (c3 == p0) {
      for (ap_uint<3> c4 = 0; c4 <= 3; c4 += 1) {
        // access_coalesce
        for (int c5 = 0; c5 <= min(7, -8 * p0 - 64 * c1 + 83); c5 += 1) {
        #pragma HLS PIPELINE II=1
          {
            B_t4 in_data;
            B_t4 out_data;
...
/* Module Definition */
void C_IO_L2_in(hls::stream<C_t16> &fifo_C_in, hls::stream<C_t16> &fifo_C_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  /* Variable Declaration */
  ...
  for (ap_uint<2> c1 = 0; c1 <= 1; c1 += 1) {
    ...
    for (int c3 = 0; c3 <= min(7, -8 * c1 + 10); c3 += 1) {
      // io_L1
      for (int c4 = 0; c4 <= min(31, -256 * c1 - 32 * c3 + 335); c4 += 1) {
        // access_coalesce
        // access_serialize
        for (ap_uint<5> c5 = 0; c5 <= 15; c5 += 1) {
        #pragma HLS PIPELINE II=1
...
/* Module Definition */
void C_IO_L2_in_serialize(C_t16 *C, hls::stream<C_t16> &fifo_C_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  /* Variable Declaration */
...
#pragma HLS INLINE OFF
  ...
  for (int c3 = p0; c3 <= min(7, -8 * c1 + 10); c3 += 1) {
    // io_L1
    if (c3 == p0) {
      for (int c4 = 0; c4 <= min(31, -32 * p0 - 256 * c1 + 335); c4 += 1) {
        // access_coalesce
        for (ap_uint<5> c5 = 0; c5 <= 15; c5 += 1) {
        #pragma HLS PIPELINE II=1
          {
...
/* Module Definition */
void C_IO_L1_in(int idx, hls::stream<C_t16> &fifo_C_in, hls::stream<C_t16> &fifo_C_out, hls::stream<C_t16> &fifo_C_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  C_t16 local_C_ping[32][16];
  #pragma HLS RESOURCE variable=local_C_ping core=RAM_1P_BRAM
  C_t16 local_C_pong[32][16];
...
/* Module Definition */
void C_IO_L1_in_boundary(int idx, hls::stream<C_t16> &fifo_C_in, hls::stream<C_t16> &fifo_C_local_out) {
#pragma HLS INLINE
  /* Variable Declaration */
  int p0 = idx; // module id
  C_t16 local_C_ping[32][16];
...
/* Module Definition */
void PE(int idx, hls::stream<A_t16> &fifo_A_in, hls::stream<A_t16> &fifo_A_out, hls::stream<float> &fifo_B_in, hls::stream<C_t16> &fifo_C_in, hls::stream<float> &fifo_D_drain_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  A_t1 local_A[1][1][16];
    ...
    for (ap_uint<7> c2 = 0; c2 <= 63; c2 += 1) {
      ...
      for (ap_uint<5> c4 = 0; c4 <= 15; c4 += 1) {
        if (c2 == 0) {
          ...
          for (ap_uint<5> c6 = 0; c6 <= 15; c6 += 1) {
            // latency
            for (int c7 = 0; c7 <= min(31, -32 * p0 - 256 * c1 + 335); c7 += 1) {
            #pragma HLS PIPELINE II=1
              local_D[16*c4 + c6][c7] = 0;
            }
          }
        }
        for (ap_uint<3> c5 = 0; c5 <= 3; c5 += 1)
              ...
              #pragma HLS PIPELINE II=1
                {
                  ...
                  #pragma HLS UNROLL
                    local_D[16*c4 + c7][c8] = (local_D[16*c4 + c7][c8] + ((local_A[0][0][c9] * local_B[0][0]) * local_C[0][c9]));
                  }
                  if (c2 == 63 && c5 == 3 && c6 == 15)
                    fifo_D_drain_out.write(local_D[16*c4 + c7][c8]);
                  {
                    A_t16 fifo_data;
...
/* Module Definition */
void A_PE_dummy_in(int idx, hls::stream<A_t16> &fifo_A_in) {
  /* Variable Declaration */
  int p0 = idx; // module id
  /* Variable Declaration */
    ...
    for (ap_uint<5> c7 = 0; c7 <= 15; c7 += 1) {
      ...
      #pragma HLS PIPELINE II=1
        // simd
        {
          ...
          int split_idx = (c8) % 4;
          out_data = local_D[16*c4 + c7][c8 / 4];
          for (ap_uint<3> n = 0; n < 4; n++) {
          #pragma HLS UNROLL
          ...
          out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);          local_D[16*c4 + c7][c8 / 4] = out_data;
        }
      }
    }
  }
