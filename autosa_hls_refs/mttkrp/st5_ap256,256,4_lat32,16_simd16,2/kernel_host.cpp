// AutoSA Command:
// /workspaces/mlir-systolic/third_party/AutoSA/autosa /workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/mttkrp/kernel.c --config=/workspaces/mlir-systolic/third_party/AutoSA/autosa_config/autosa_config.json --target=autosa_hls_c --output-dir=/workspaces/mlir-systolic/third_party/AutoSA/autosa.tmp/output --host-serialize --simd-info=/workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/mttkrp/simd_info.json --sa-sizes={kernel[]->space_time[5];kernel[]->array_part[256,256,4];kernel[]->latency[32,16];kernel[]->simd[16,2]}
...
//      C[l][j] = 2.5;
  ...
  // computation
  {
    ...
    std::vector<float, aligned_allocator<float>> dev_A_unserialized((256) * (256) * (256));
    std::vector<float, aligned_allocator<float>> dev_A(33554432);
    std::vector<float, aligned_allocator<float>> dev_B_unserialized((256) * (336));
    std::vector<float, aligned_allocator<float>> dev_B(5505024);
    std::vector<float, aligned_allocator<float>> dev_C_unserialized((336) * (256));
    std::vector<float, aligned_allocator<float>> dev_C(1409286144);
    std::vector<float, aligned_allocator<float>> dev_D_unserialized((256) * (336));
    std::vector<float, aligned_allocator<float>> dev_D(86016);

    // Initialize host buffers
    std::copy(reinterpret_cast<float *>(A), reinterpret_cast<float *>(A) + (256) * (256) * (256), dev_A_unserialized.begin());
    std::copy(reinterpret_cast<float *>(B), reinterpret_cast<float *>(B) + (256) * (336), dev_B_unserialized.begin());
    std::copy(reinterpret_cast<float *>(C), reinterpret_cast<float *>(C) + (336) * (256), dev_C_unserialized.begin());
