// AutoSA Command:
// /workspaces/mlir-systolic/third_party/AutoSA/autosa /workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/mttkrp/kernel.c --config=/workspaces/mlir-systolic/third_party/AutoSA/autosa_config/autosa_config.json --target=autosa_hls_c --output-dir=/workspaces/mlir-systolic/third_party/AutoSA/autosa.tmp/output --host-serialize --simd-info=/workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/mttkrp/simd_info.json --sa-sizes={kernel[]->space_time[1];kernel[]->array_part[64,64,2];kernel[]->latency[8,4];kernel[]->simd[4,1]}
...
void A_IO_L2_in_intra_trans(int c0, int c1, int c2, A_t16 local_A[64][2][16], hls::stream<A_t4> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L2_in_inter_trans_boundary(int c0, int c1, int c2, A_t16 local_A[64][2][16], hls::stream<A_t16> &fifo_A_in, bool inter_trans_en);
void B_IO_L1_in_intra_trans(int idx, int c0, int c1, int c2, B_t4 local_B[2][2], hls::stream<float> &fifo_B_local_out, bool intra_trans_en);
void B_IO_L1_in_inter_trans(int idx, int c0, int c1, int c2, B_t4 local_B[2][2], hls::stream<B_t4> &fifo_B_in, hls::stream<B_t4> &fifo_B_out, bool inter_trans_en);
void B_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c2, B_t4 local_B[2][2], hls::stream<B_t4> &fifo_B_in, bool inter_trans_en);
...
void C_IO_L1_in_intra_trans(int idx, int c0, int c1, C_t4 local_C[8][64], hls::stream<C_t4> &fifo_C_local_out, bool intra_trans_en);
void C_IO_L1_in_inter_trans(int idx, int c0, int c1, C_t4 local_C[8][64], hls::stream<C_t4> &fifo_C_in, hls::stream<C_t4> &fifo_C_out, bool inter_trans_en);
void C_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, C_t4 local_C[8][64], hls::stream<C_t4> &fifo_C_in, bool inter_trans_en);
...
void PE_wrapper(int idx, hls::stream<A_t4> &fifo_A_in, hls::stream<A_t4> &fifo_A_out, hls::stream<float> &fifo_B_in, hls::stream<C_t4> &fifo_C_in, hls::stream<float> &fifo_D_drain_out);
void D_drain_IO_L1_out_intra_trans(int idx, int c0, int c1, D_t4 local_D[64][2], hls::stream<float> &fifo_D_drain_local_in);
void D_drain_IO_L1_out_inter_trans(int idx, int c0, int c1, D_t4 local_D[64][2], hls::stream<D_t4> &fifo_D_drain_in, hls::stream<D_t4> &fifo_D_drain_out);
void D_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c1, D_t4 local_D[64][2], hls::stream<D_t4> &fifo_D_drain_out);
        ...
        for (ap_uint<3> c7 = 0; c7 <= 3; c7 += 1) {
          ...
          #pragma HLS PIPELINE II=1
            // simd
            {
              A_t16 in_data;
              A_t4 out_data;
              in_data = local_A[4*c4 + c7][c5][4 * c6 / 16];
...
/* Module Definition */
void A_IO_L2_in_boundary(hls::stream<A_t16> &fifo_A_in, hls::stream<A_t4> &fifo_A_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  A_t16 local_A_ping[64][2][16];
  #pragma HLS RESOURCE variable=local_A_ping core=RAM_2P_BRAM
  A_t16 local_A_pong[64][2][16];
  ...
  for (ap_uint<3> c0 = 0; c0 <= 3; c0 += 1)
    for (ap_uint<4> c1 = 0; c1 <= 5; c1 += 1)
      for (ap_uint<8> c2 = 0; c2 <= 127; c2 += 1) {
        // array
        // io_L2
        for (int c3 = 0; c3 <= min(7, -8 * c1 + 41); c3 += 1) {
          // io_L1
          for (ap_uint<2> c4 = 0; c4 <= 1; c4 += 1) {
            // access_coalesce
            // access_serialize
...
#pragma HLS INLINE OFF
  ...
  for (int c3 = p0; c3 <= min(7, -8 * c1 + 41); c3 += 1) {
    // io_L1
    if (c3 == p0) {
      for (ap_uint<2> c4 = 0; c4 <= 1; c4 += 1) {
        // access_coalesce
...
/* Module Definition */
void PE(int idx, hls::stream<A_t4> &fifo_A_in, hls::stream<A_t4> &fifo_A_out, hls::stream<float> &fifo_B_in, hls::stream<C_t4> &fifo_C_in, hls::stream<float> &fifo_D_drain_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  A_t1 local_A[1][1][4];
                  ...
                  #pragma HLS PIPELINE II=1
                    {
                      ...
                      #pragma HLS UNROLL
                        local_D[4*c4 + c7][c8] = (local_D[4*c4 + c7][c8] + ((local_A[0][0][c9] * local_B[0][0]) * local_C[0][c9]));
                      }
                      if (c2 == 127 && c5 == 1 && c6 == 63)
                        fifo_D_drain_out.write(local_D[4*c4 + c7][c8]);
                      {
                        A_t4 fifo_data;
...
/* Module Definition */
void A_PE_dummy_in(int idx, hls::stream<A_t4> &fifo_A_in) {
  /* Variable Declaration */
  int p0 = idx; // module id
  /* Variable Declaration */
    ...
    for (ap_uint<3> c7 = 0; c7 <= 3; c7 += 1) {
      ...
      #pragma HLS PIPELINE II=1
        // simd
        {
          ...
          out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);          local_D[4*c4 + c7][c8 / 4] = out_data;
        }
      }
    }
  }
