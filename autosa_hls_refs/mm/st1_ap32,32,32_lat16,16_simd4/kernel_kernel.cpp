...
void A_IO_L2_in_intra_trans(int c0, int c1, int c2, A_t16 local_A[32][2], hls::stream<A_t4> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L2_in_inter_trans_boundary(int c0, int c1, int c2, A_t16 local_A[32][2], hls::stream<A_t16> &fifo_A_in, bool inter_trans_en);
void B_IO_L1_in_intra_trans(int idx, int c0, int c1, int c2, B_t4 local_B[16][8], hls::stream<B_t4> &fifo_B_local_out, bool intra_trans_en);
void B_IO_L1_in_inter_trans(int idx, int c0, int c1, int c2, B_t4 local_B[16][8], hls::stream<B_t4> &fifo_B_in, hls::stream<B_t4> &fifo_B_out, bool inter_trans_en);
void B_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c2, B_t4 local_B[16][8], hls::stream<B_t4> &fifo_B_in, bool inter_trans_en);
...
void PE_wrapper(int idx, hls::stream<A_t4> &fifo_A_in, hls::stream<A_t4> &fifo_A_out, hls::stream<B_t4> &fifo_B_in, hls::stream<float> &fifo_C_drain_out);
void C_drain_IO_L1_out_intra_trans(int idx, int c0, int c1, C_t4 local_C[32][4], hls::stream<float> &fifo_C_drain_local_in);
void C_drain_IO_L1_out_inter_trans(int idx, int c0, int c1, C_t4 local_C[32][4], hls::stream<C_t4> &fifo_C_drain_in, hls::stream<C_t4> &fifo_C_drain_out);
void C_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c1, C_t4 local_C[32][4], hls::stream<C_t4> &fifo_C_drain_out);
      ...
      for (ap_uint<5> c6 = 0; c6 <= 15; c6 += 1) {
        ...
        #pragma HLS PIPELINE II=1
          // simd
          {
            A_t16 in_data;
            A_t4 out_data;
            in_data = local_A[16*c4 + c6][4 * c5 / 16];
...
/* Module Definition */
void PE(int idx, hls::stream<A_t4> &fifo_A_in, hls::stream<A_t4> &fifo_A_out, hls::stream<B_t4> &fifo_B_in, hls::stream<float> &fifo_C_drain_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  A_t1 local_A[1][4];
              ...
              #pragma HLS PIPELINE II=1
                {
                  ...
                  #pragma HLS UNROLL
                    local_C[16*c4 + c6][c7] = (local_C[16*c4 + c6][c7] + (local_A[0][c8] * local_B[0][c8]));
                  }
                  if (c2 == 1 && c5 == 7)
                    fifo_C_drain_out.write(local_C[16*c4 + c6][c7]);
                  {
                    A_t4 fifo_data;
    ...
    for (ap_uint<5> c6 = 0; c6 <= 15; c6 += 1) {
      ...
      #pragma HLS PIPELINE II=1
        // simd
        {
          ...
          int split_idx = (c7) % 4;
          out_data = local_C[16*c4 + c6][c7 / 4];
          for (ap_uint<3> n = 0; n < 4; n++) {
          #pragma HLS UNROLL
          ...
          out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);          local_C[16*c4 + c6][c7 / 4] = out_data;
        }
      }
    }
  }
