// AutoSA Command:
// /workspaces/mlir-systolic/third_party/AutoSA/autosa /workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/ttmc/kernel.c --config=/workspaces/mlir-systolic/third_party/AutoSA/autosa_config/autosa_config.json --target=autosa_hls_c --output-dir=/workspaces/mlir-systolic/third_party/AutoSA/autosa.tmp/output --host-serialize --simd-info=/workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/ttmc/simd_info.json --sa-sizes={kernel[]->space_time[2];kernel[]->array_part[64,64,64,64];kernel[]->latency[4,8,8];kernel[]->simd[4,4]}
...
void A_IO_L1_in_intra_trans(int idx, int c0, int c1, int c2, int c3, A_t4 local_A[64][64][32], hls::stream<A_t4> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L1_in_inter_trans(int idx, int c0, int c1, int c2, int c3, A_t4 local_A[64][64][32], hls::stream<A_t4> &fifo_A_in, hls::stream<A_t4> &fifo_A_out, bool inter_trans_en);
void A_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c2, int c3, A_t4 local_A[64][64][32], hls::stream<A_t4> &fifo_A_in, bool inter_trans_en);
...
void B_IO_L1_in_intra_trans(int idx, int c0, int c1, int c2, int c3, B_t4 local_B[64][16], hls::stream<float> &fifo_B_local_out, bool intra_trans_en);
void B_IO_L1_in_inter_trans(int idx, int c0, int c1, int c2, int c3, B_t4 local_B[64][16], hls::stream<B_t4> &fifo_B_in, hls::stream<B_t4> &fifo_B_out, bool inter_trans_en);
void B_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c2, int c3, B_t4 local_B[64][16], hls::stream<B_t4> &fifo_B_in, bool inter_trans_en);
...
void C_IO_L1_in_intra_trans(int idx, int c0, int c1, int c2, C_t4 local_C[4][32], hls::stream<C_t4> &fifo_C_local_out, bool intra_trans_en);
void C_IO_L1_in_inter_trans(int idx, int c0, int c1, int c2, C_t4 local_C[4][32], hls::stream<C_t4> &fifo_C_in, hls::stream<C_t4> &fifo_C_out, bool inter_trans_en);
void C_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c2, C_t4 local_C[4][32], hls::stream<C_t4> &fifo_C_in, bool inter_trans_en);
...
void PE_wrapper(int idx, hls::stream<A_t4> &fifo_A_in, hls::stream<float> &fifo_B_in, hls::stream<C_t4> &fifo_C_in, hls::stream<float> &fifo_D_drain_out);
void D_drain_IO_L1_out_intra_trans(int idx, int c0, int c1, int c2, D_t4 local_D[64][64][1], hls::stream<float> &fifo_D_drain_local_in);
void D_drain_IO_L1_out_inter_trans(int idx, int c0, int c1, int c2, D_t4 local_D[64][64][1], hls::stream<D_t4> &fifo_D_drain_in, hls::stream<D_t4> &fifo_D_drain_out);
void D_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c1, int c2, D_t4 local_D[64][64][1], hls::stream<D_t4> &fifo_D_drain_out);
            ...
            for (ap_uint<4> c10 = 0; c10 <= 7; c10 += 1) {
              ...
              #pragma HLS PIPELINE II=1
                // simd
                {
                  A_t4 in_data;
                  A_t4 out_data;
                  in_data = local_A[c10 + 8*c5][c7][c8];
                  out_data = in_data;
                  fifo_A_local_out.write(out_data);
                }
              }
...
/* Module Definition */
void PE(int idx, hls::stream<A_t4> &fifo_A_in, hls::stream<float> &fifo_B_in, hls::stream<C_t4> &fifo_C_in, hls::stream<float> &fifo_D_drain_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  A_t1 local_A[1][1][4];
                      ...
                      #pragma HLS PIPELINE II=1
                        {
                          ...
                          #pragma HLS UNROLL
                            local_D[c10 + 8*c5][8*c6 + c9][c11] = (local_D[c10 + 8*c5][8*c6 + c9][c11] + ((local_A[0][0][c12] * local_B[0][0]) * local_C[0][c12]));
                          }
                          if (c3 == 1 && c7 == 63 && c8 == 31)
                            fifo_D_drain_out.write(local_D[c10 + 8*c5][8*c6 + c9][c11]);
                        }
                      }
                    }
                  }
        ...
        for (ap_uint<4> c10 = 0; c10 <= 7; c10 += 1) {
          ...
          #pragma HLS PIPELINE II=1
            // simd
            {
              ...
              int split_idx = (c11) % 4;
              out_data = local_D[c10 + 8*c5][8*c6 + c9][c11 / 4];
              for (ap_uint<3> n = 0; n < 4; n++) {
              #pragma HLS UNROLL
              ...
              out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);              local_D[c10 + 8*c5][8*c6 + c9][c11 / 4] = out_data;
            }
          }
        }
      }
