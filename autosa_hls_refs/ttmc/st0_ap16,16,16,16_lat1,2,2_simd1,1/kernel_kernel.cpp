// AutoSA Command:
// /workspaces/mlir-systolic/third_party/AutoSA/autosa /workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/ttmc/kernel.c --config=/workspaces/mlir-systolic/third_party/AutoSA/autosa_config/autosa_config.json --target=autosa_hls_c --output-dir=/workspaces/mlir-systolic/third_party/AutoSA/autosa.tmp/output --host-serialize --simd-info=/workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/ttmc/simd_info.json --sa-sizes={kernel[]->space_time[0];kernel[]->array_part[16,16,16,16];kernel[]->latency[1,2,2];kernel[]->simd[1,1]}
...
void A_IO_L1_in_intra_trans(int idx, int c0, int c1, int c2, int c3, A_t4 local_A[1][16][32], hls::stream<float> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L1_in_inter_trans(int idx, int c0, int c1, int c2, int c3, A_t4 local_A[1][16][32], hls::stream<A_t4> &fifo_A_in, hls::stream<A_t4> &fifo_A_out, bool inter_trans_en);
void A_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c2, int c3, A_t4 local_A[1][16][32], hls::stream<A_t4> &fifo_A_in, bool inter_trans_en);
...
void B_IO_L2_in_intra_trans(int c0, int c1, int c2, int c3, B_t16 local_B[16][1], hls::stream<float> &fifo_B_local_out, bool intra_trans_en);
void B_IO_L2_in_inter_trans_boundary(int c0, int c1, int c2, int c3, B_t16 local_B[16][1], hls::stream<B_t16> &fifo_B_in, bool inter_trans_en);
void C_IO_L2_in_intra_trans(int c0, int c1, int c2, int c3, C_t16 local_C[16][8], hls::stream<float> &fifo_C_local_out, bool intra_trans_en);
void C_IO_L2_in_inter_trans_boundary(int c0, int c1, int c2, C_t16 local_C[16][8], hls::stream<C_t16> &fifo_C_in, bool inter_trans_en);
void PE_wrapper(int idx, hls::stream<float> &fifo_A_in, hls::stream<float> &fifo_B_in, hls::stream<float> &fifo_B_out, hls::stream<float> &fifo_C_in, hls::stream<float> &fifo_C_out, hls::stream<float> &fifo_D_drain_out);
void D_drain_IO_L1_out_intra_trans(int idx, int c0, int c1, int c2, D_t4 local_D[1][16][4], hls::stream<float> &fifo_D_drain_local_in);
void D_drain_IO_L1_out_inter_trans(int idx, int c0, int c1, int c2, D_t4 local_D[1][16][4], hls::stream<D_t4> &fifo_D_drain_in, hls::stream<D_t4> &fifo_D_drain_out);
void D_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c1, int c2, D_t4 local_D[1][16][4], hls::stream<D_t4> &fifo_D_drain_out);
            ...
            #pragma HLS PIPELINE II=1
              {
                B_t16 in_data;
                B_t1 out_data;
                in_data = local_B[c7][(2 * c5 + c10) / 16];
                for (ap_uint<5> n = 0; n < 16; n++) {
                #pragma HLS UNROLL
                  data_split[n] = in_data(31, 0);
                  in_data = in_data >> 32;
                }
                int split_idx = (c10 + 2*c5) % 16;
                union {unsigned int ui; float ut;} u;
                u.ui = (unsigned int)data_split[split_idx];
                out_data = u.ut;
...
/* Module Definition */
void C_IO_L2_in_boundary(hls::stream<C_t16> &fifo_C_in, hls::stream<float> &fifo_C_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
...
/* Module Definition */
void PE(int idx, hls::stream<float> &fifo_A_in, hls::stream<float> &fifo_B_in, hls::stream<float> &fifo_B_out, hls::stream<float> &fifo_C_in, hls::stream<float> &fifo_C_out, hls::stream<float> &fifo_D_drain_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  A_t1 local_A[1][1][1];
                    ...
                    #pragma HLS PIPELINE II=1
                      {
                        ...
                        local_D[0][c10 + 2*c5][2*c6 + c9] = (local_D[0][c10 + 2*c5][2*c6 + c9] + ((local_A[0][0][0] * local_B[0][0]) * local_C[0][0]));
                        if (c3 == 7 && c7 == 15 && c8 == 127)
                          fifo_D_drain_out.write(local_D[0][c10 + 2*c5][2*c6 + c9]);
...
/* Module Definition */
void C_PE_dummy_in(int idx, hls::stream<float> &fifo_C_in) {
  /* Variable Declaration */
  int p0 = idx; // module id
  /* Variable Declaration */
        ...
        #pragma HLS PIPELINE II=1
          {
            ...
            out_data = local_D[0][c10 + 2*c5][(2 * c6 + c9) / 4];
            for (ap_uint<3> n = 0; n < 4; n++) {
            #pragma HLS UNROLL
            ...
            out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);            local_D[0][c10 + 2*c5][(2 * c6 + c9) / 4] = out_data;
          }
        }
      }
    }
...
#pragma HLS DATAFLOW
  ...
  /* PE fifo */ hls::stream<float> fifo_C_PE_16;
