// AutoSA Command:
// /workspaces/mlir-systolic/third_party/AutoSA/autosa /workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/ttmc/kernel.c --config=/workspaces/mlir-systolic/third_party/AutoSA/autosa_config/autosa_config.json --target=autosa_hls_c --output-dir=/workspaces/mlir-systolic/third_party/AutoSA/autosa.tmp/output --host-serialize --simd-info=/workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/ttmc/simd_info.json --sa-sizes={kernel[]->space_time[1];kernel[]->array_part[64,64,64,64];kernel[]->latency[4,8,8];kernel[]->simd[4,4]}
...
void A_IO_L2_in_intra_trans(int c0, int c1, int c2, int c3, A_t16 local_A[64][64][8], hls::stream<A_t4> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L2_in_inter_trans_boundary(int c0, int c1, int c2, int c3, A_t16 local_A[64][64][8], hls::stream<A_t16> &fifo_A_in, bool inter_trans_en);
void B_IO_L1_in_intra_trans(int idx, int c0, int c1, int c2, int c3, B_t4 local_B[64][1], hls::stream<float> &fifo_B_local_out, bool intra_trans_en);
void B_IO_L1_in_inter_trans(int idx, int c0, int c1, int c2, int c3, B_t4 local_B[64][1], hls::stream<B_t4> &fifo_B_in, hls::stream<B_t4> &fifo_B_out, bool inter_trans_en);
void B_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c2, int c3, B_t4 local_B[64][1], hls::stream<B_t4> &fifo_B_in, bool inter_trans_en);
...
void D_drain_IO_L1_out_intra_trans(int idx, int c0, int c1, int c2, D_t4 local_D[64][4][16], hls::stream<float> &fifo_D_drain_local_in);
void D_drain_IO_L1_out_inter_trans(int idx, int c0, int c1, int c2, D_t4 local_D[64][4][16], hls::stream<D_t4> &fifo_D_drain_in, hls::stream<D_t4> &fifo_D_drain_out);
void D_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c1, int c2, D_t4 local_D[64][4][16], hls::stream<D_t4> &fifo_D_drain_out);
            ...
            for (ap_uint<4> c10 = 0; c10 <= 7; c10 += 1) {
              ...
              #pragma HLS PIPELINE II=1
                // simd
                {
                  A_t16 in_data;
                  A_t4 out_data;
                  in_data = local_A[c10 + 8*c5][c7][4 * c8 / 16];
                      ...
                      #pragma HLS PIPELINE II=1
                        {
                          ...
                          #pragma HLS UNROLL
                            local_D[c10 + 8*c5][c11][8*c6 + c9] = (local_D[c10 + 8*c5][c11][8*c6 + c9] + ((local_A[0][0][c12] * local_B[0][0]) * local_C[0][c12]));
                          }
                          if (c3 == 1 && c7 == 63 && c8 == 31)
                            fifo_D_drain_out.write(local_D[c10 + 8*c5][c11][8*c6 + c9]);
                          {
                            A_t4 fifo_data;
        ...
        for (ap_uint<4> c10 = 0; c10 <= 7; c10 += 1) {
          ...
          #pragma HLS PIPELINE II=1
            // simd
            {
              ...
              out_data = local_D[c10 + 8*c5][c11][(8 * c6 + c9) / 4];
              for (ap_uint<3> n = 0; n < 4; n++) {
              #pragma HLS UNROLL
              ...
              out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);              local_D[c10 + 8*c5][c11][(8 * c6 + c9) / 4] = out_data;
            }
          }
        }
      }
