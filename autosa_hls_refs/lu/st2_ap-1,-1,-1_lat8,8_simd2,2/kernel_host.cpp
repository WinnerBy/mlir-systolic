...
{
  {
    ...
    std::vector<float, aligned_allocator<float>> dev_A_unserialized((32) * (32));
    std::vector<float, aligned_allocator<float>> dev_A(1024);
    std::vector<float, aligned_allocator<float>> dev_L_unserialized((31) * (32));
    std::vector<float, aligned_allocator<float>> dev_L(496);
    std::vector<float, aligned_allocator<float>> dev_U_unserialized((32) * (32));
    std::vector<float, aligned_allocator<float>> dev_U(1024);

    // Initialize host buffers
    std::copy(reinterpret_cast<float *>(A), reinterpret_cast<float *>(A) + (32) * (32), dev_A_unserialized.begin());
    std::copy(reinterpret_cast<float *>(L), reinterpret_cast<float *>(L) + (31) * (32), dev_L_unserialized.begin());
    std::copy(reinterpret_cast<float *>(U), reinterpret_cast<float *>(U) + (32) * (32), dev_U_unserialized.begin());
    ...
    host_deserialize_L(dev_L_unserialized, dev_L);
    host_deserialize_U(dev_U_unserialized, dev_U);
    // Restore data from host buffers
    std::copy(dev_L_unserialized.begin(), dev_L_unserialized.end(), reinterpret_cast<float *>(L));
    std::copy(dev_U_unserialized.begin(), dev_U_unserialized.end(), reinterpret_cast<float *>(U));
  }
}
