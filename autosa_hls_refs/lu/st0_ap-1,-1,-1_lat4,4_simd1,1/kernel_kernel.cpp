...
template<int p0>
void A_IO_L1_in_intra_trans(A_t4 local_A[32][8], hls::stream<float> &fifo_A_local_out);
template<int p0>
void A_IO_L1_in_inter_trans(A_t4 local_A[32][8], hls::stream<A_t4> &fifo_A_in, hls::stream<A_t4> &fifo_A_out);
template<int p0>
void A_IO_L1_in_inter_trans_boundary(A_t4 local_A[32][8], hls::stream<A_t4> &fifo_A_in);
template<int p0>
...
template<int p0>
void PE_wrapper(hls::stream<float> &fifo_V_in, hls::stream<float> &fifo_V_out, hls::stream<float> &fifo_A_in, hls::stream<float> &fifo_L_drain_out, hls::stream<float> &fifo_U_drain_out);
template<int p0>
...
/* Module Definition */
template<int p0>
void PE(hls::stream<float> &fifo_V_in, hls::stream<float> &fifo_V_out, hls::stream<float> &fifo_A_in, hls::stream<float> &fifo_L_drain_out, hls::stream<float> &fifo_U_drain_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  prev_V_t1 local_prev_V[32][32];
  #pragma HLS RESOURCE variable=local_prev_V core=RAM_2P_BRAM
  V_t1 local_V[32][32];
  #pragma HLS RESOURCE variable=local_V core=RAM_2P_BRAM
  U_tmp_t1 local_U_tmp[32][32];
  #pragma HLS RESOURCE variable=local_U_tmp core=RAM_2P_BRAM
  L_tmp_t1 local_L_tmp[32][31];
  ...
  // array
  // pe
  for (ap_uint<6> c1 = p0; c1 <= 31; c1 += 1)
    for (ap_uint<6> c2 = p0; c2 <= 31; c2 += 1) {
    #pragma HLS PIPELINE II=1
      {
        if (p0 == 0) {
          local_A[0][0] = fifo_A_in.read();
          local_prev_V[c2][c1] = local_A[0][0];
        } else {
          local_V[c2 - p0][c1 - p0] = fifo_V_in.read();
          local_prev_V[c2 - p0][c1 - p0] = local_V[c2 - p0][c1 - p0];
        }
        if (c1 == p0) {
          local_U_tmp[c2 - p0][0] = local_prev_V[c2 - p0][0];
          local_U[0][0] = local_U_tmp[c2 - p0][0];
          fifo_U_drain_out.write(local_U[0][0]);
        } else {
          local_U_tmp[c2 - p0][c1 - p0] = local_U_tmp[c2 - p0][c1 - p0 - 1];
          if (c2 == p0) {
            local_L_tmp[0][c1 - p0 - 1] = (local_prev_V[0][c1 - p0] / local_U_tmp[0][c1 - p0]);
            local_L[0][0] = local_L_tmp[0][c1 - p0 - 1];
            fifo_L_drain_out.write(local_L[0][0]);
          } else {
            local_L_tmp[c2 - p0][c1 - p0 - 1] = local_L_tmp[c2 - p0 - 1][c1 - p0 - 1];
          }
          local_V[c2 - p0][c1 - p0] = (local_prev_V[c2 - p0][c1 - p0] - (local_L_tmp[c2 - p0][c1 - p0 - 1] * local_U_tmp[c2 - p0][c1 - p0]));
          if (c2 >= p0 + 1)
            fifo_V_out.write(local_V[c2 - p0][c1 - p0]);
        }
      }
    }
}
...
#pragma HLS INLINE
  ...
  // io_L1
  // pe
  for (ap_uint<6> c1 = p0 + 1; c1 <= 31; c1 += 1) {
  #pragma HLS PIPELINE II=1
    // hls_pipeline
    {
      L_t1 in_data;
...
#pragma HLS DATAFLOW
  ...
  /* PE fifo */ hls::stream<float> fifo_V_PE_0;
  #pragma HLS STREAM variable=fifo_V_PE_0 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_0 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_1;
  #pragma HLS STREAM variable=fifo_V_PE_1 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_1 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_2;
  #pragma HLS STREAM variable=fifo_V_PE_2 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_2 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_3;
  #pragma HLS STREAM variable=fifo_V_PE_3 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_3 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_4;
  #pragma HLS STREAM variable=fifo_V_PE_4 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_4 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_5;
  #pragma HLS STREAM variable=fifo_V_PE_5 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_5 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_6;
  #pragma HLS STREAM variable=fifo_V_PE_6 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_6 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_7;
  #pragma HLS STREAM variable=fifo_V_PE_7 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_7 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_8;
  #pragma HLS STREAM variable=fifo_V_PE_8 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_8 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_9;
  #pragma HLS STREAM variable=fifo_V_PE_9 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_9 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_10;
  #pragma HLS STREAM variable=fifo_V_PE_10 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_10 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_11;
  #pragma HLS STREAM variable=fifo_V_PE_11 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_11 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_12;
  #pragma HLS STREAM variable=fifo_V_PE_12 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_12 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_13;
  #pragma HLS STREAM variable=fifo_V_PE_13 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_13 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_14;
  #pragma HLS STREAM variable=fifo_V_PE_14 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_14 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_15;
  #pragma HLS STREAM variable=fifo_V_PE_15 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_15 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_16;
  #pragma HLS STREAM variable=fifo_V_PE_16 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_16 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_17;
  #pragma HLS STREAM variable=fifo_V_PE_17 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_17 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_18;
  #pragma HLS STREAM variable=fifo_V_PE_18 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_18 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_19;
  #pragma HLS STREAM variable=fifo_V_PE_19 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_19 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_20;
  #pragma HLS STREAM variable=fifo_V_PE_20 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_20 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_21;
  #pragma HLS STREAM variable=fifo_V_PE_21 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_21 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_22;
  #pragma HLS STREAM variable=fifo_V_PE_22 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_22 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_23;
  #pragma HLS STREAM variable=fifo_V_PE_23 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_23 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_24;
  #pragma HLS STREAM variable=fifo_V_PE_24 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_24 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_25;
  #pragma HLS STREAM variable=fifo_V_PE_25 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_25 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_26;
  #pragma HLS STREAM variable=fifo_V_PE_26 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_26 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_27;
  #pragma HLS STREAM variable=fifo_V_PE_27 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_27 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_28;
  #pragma HLS STREAM variable=fifo_V_PE_28 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_28 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_29;
  #pragma HLS STREAM variable=fifo_V_PE_29 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_29 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_30;
  #pragma HLS STREAM variable=fifo_V_PE_30 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_30 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_31;
  #pragma HLS STREAM variable=fifo_V_PE_31 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_31 core=FIFO_SRL
  /* PE fifo */ hls::stream<float> fifo_V_PE_32;
  #pragma HLS STREAM variable=fifo_V_PE_32 depth=2
  #pragma HLS RESOURCE variable=fifo_V_PE_32 core=FIFO_SRL
