// AutoSA Command:
// /workspaces/mlir-systolic/third_party/AutoSA/autosa /workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/cnn/kernel.c --config=/workspaces/mlir-systolic/third_party/AutoSA/autosa_config/autosa_config.json --target=autosa_hls_c --output-dir=/workspaces/mlir-systolic/third_party/AutoSA/autosa.tmp/output --host-serialize --simd-info=/workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/cnn/simd_info.json --sa-sizes={kernel[]->space_time[1];kernel[]->array_part[128,112,28,128];kernel[]->latency[8,8,14];kernel[]->simd[2,2,16]}
...
void cin_IO_L1_in_intra_trans(int idx, int c0, int c2, int c3, cin_t16 local_cin[10][30][8], hls::stream<cin_t16> &fifo_cin_local_out, bool intra_trans_en);
void cin_IO_L1_in_inter_trans(int idx, int c0, int c2, int c3, cin_t16 local_cin[10][30][8], hls::stream<cin_t16> &fifo_cin_in, hls::stream<cin_t16> &fifo_cin_out, bool inter_trans_en);
void cin_IO_L1_in_inter_trans_boundary(int idx, int c0, int c2, int c3, cin_t16 local_cin[10][30][8], hls::stream<cin_t16> &fifo_cin_in, bool inter_trans_en);
...
void w_IO_L2_in_intra_trans(int c0, int c2, int c3, w_t16 local_w[112][3][3][8], hls::stream<w_t16> &fifo_w_local_out, bool intra_trans_en);
void w_IO_L2_in_inter_trans_boundary(int c0, int c2, int c3, w_t16 local_w[112][3][3][8], hls::stream<w_t16> &fifo_w_in, bool inter_trans_en);
void PE_wrapper(int idx, hls::stream<cin_t16> &fifo_cin_in, hls::stream<float> &fifo_cout_drain_out, hls::stream<w_t16> &fifo_w_in, hls::stream<w_t16> &fifo_w_out);
void cout_drain_IO_L1_out_intra_trans(int idx, int c0, int c2, cout_t4 local_cout[8][28][28], hls::stream<float> &fifo_cout_drain_local_in);
void cout_drain_IO_L1_out_inter_trans(int idx, int c0, int c2, cout_t4 local_cout[8][28][28], hls::stream<cout_t4> &fifo_cout_drain_in, hls::stream<cout_t4> &fifo_cout_drain_out);
void cout_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c2, cout_t4 local_cout[8][28][28], hls::stream<cout_t4> &fifo_cout_drain_out);
              ...
              for (ap_uint<4> c11 = 0; c11 <= 7; c11 += 1) {
                ...
                #pragma HLS PIPELINE II=1
                  // simd
                  {
                    cin_t16 in_data;
                    cin_t16 out_data;
                    in_data = local_cin[c12 + c8][c10 + 14*c6 + c9][c7];
                    out_data = in_data;
                    fifo_cin_local_out.write(out_data);
                  }
                }
...
#pragma HLS INLINE OFF
  ...
  // io_L2
  for (int c4 = 0; c4 <= min(111, -112 * c0 + 639); c4 += 1)
...
/* Module Definition */
void w_IO_L2_in_boundary(hls::stream<w_t16> &fifo_w_in, hls::stream<w_t16> &fifo_w_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
...
/* Module Definition */
void PE(int idx, hls::stream<cin_t16> &fifo_cin_in, hls::stream<float> &fifo_cout_drain_out, hls::stream<w_t16> &fifo_w_in, hls::stream<w_t16> &fifo_w_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
              ...
              for (ap_uint<5> c8 = 0; c8 <= 13; c8 += 1) {
                ...
                for (ap_uint<4> c9 = 0; c9 <= 7; c9 += 1) {
                  ...
                  #pragma HLS PIPELINE II=1
                    // simd
                    // hls_unroll
                    local_cout[c10][14*c6 + c8][8*c5 + c9] = 0;
                  }
                }
              }
            }
                      ...
                      #pragma HLS PIPELINE II=1
                        {
                          ...
                          #pragma HLS UNROLL
                            local_cout[c12][c10 + 14*c6][c11 + 8*c5] = (local_cout[c12][c10 + 14*c6][c11 + 8*c5] + (local_cin[0][0][c13] * local_w[0][0][0][c13]));
                          }
                          {
                            w_t16 fifo_data;
                          ...
                          if (c3 == 3 && c7 == 7 && c8 == 2 && c9 == 2)
                            fifo_cout_drain_out.write(local_cout[c12][c10 + 14*c6][c11 + 8*c5]);
                        }
                      }
                    }
                  }
...
/* Module Definition */
void w_PE_dummy_in(int idx, hls::stream<w_t16> &fifo_w_in) {
  /* Variable Declaration */
  int p0 = idx; // module id
  /* Variable Declaration */
        ...
        for (ap_uint<4> c11 = 0; c11 <= 7; c11 += 1) {
          ...
          #pragma HLS PIPELINE II=1
            // simd
            {
              ...
              out_data = local_cout[c12][c10 + 14*c6][(8 * c5 + c11) / 4];
              ...
              out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);              local_cout[c12][c10 + 14*c6][(8 * c5 + c11) / 4] = out_data;
            }
          }
        }
      }
