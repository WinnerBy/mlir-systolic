// AutoSA Command:
// /workspaces/mlir-systolic/third_party/AutoSA/autosa /workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/cnn/kernel.c --config=/workspaces/mlir-systolic/third_party/AutoSA/autosa_config/autosa_config.json --target=autosa_hls_c --output-dir=/workspaces/mlir-systolic/third_party/AutoSA/autosa.tmp/output --host-serialize --simd-info=/workspaces/mlir-systolic/third_party/AutoSA/autosa_tests/large/cnn/simd_info.json --sa-sizes={kernel[]->space_time[2];kernel[]->array_part[64,56,14,64];kernel[]->latency[4,4,7];kernel[]->simd[1,1,8]}
...
void cin_IO_L1_in_intra_trans(int idx, int c0, int c1, int c3, cin_t8 local_cin[16][6][8], hls::stream<cin_t8> &fifo_cin_local_out, bool intra_trans_en);
void cin_IO_L1_in_inter_trans(int idx, int c0, int c1, int c3, cin_t8 local_cin[16][6][8], hls::stream<cin_t8> &fifo_cin_in, hls::stream<cin_t8> &fifo_cin_out, bool inter_trans_en);
void cin_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c3, cin_t8 local_cin[16][6][8], hls::stream<cin_t8> &fifo_cin_in, bool inter_trans_en);
...
void w_IO_L1_in_intra_trans(int idx, int c0, int c1, int c3, w_t8 local_w[56][3][3][8], hls::stream<w_t8> &fifo_w_local_out, bool intra_trans_en);
void w_IO_L1_in_inter_trans(int idx, int c0, int c1, int c3, w_t8 local_w[56][3][3][8], hls::stream<w_t8> &fifo_w_in, hls::stream<w_t8> &fifo_w_out, bool inter_trans_en);
void w_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c3, w_t8 local_w[56][3][3][8], hls::stream<w_t8> &fifo_w_in, bool inter_trans_en);
...
void PE_wrapper(int idx, hls::stream<cin_t8> &fifo_cin_in, hls::stream<float> &fifo_cout_drain_out, hls::stream<w_t8> &fifo_w_in);
void cout_drain_IO_L1_out_intra_trans(int idx, int c0, int c1, cout_t4 local_cout[14][4][14], hls::stream<float> &fifo_cout_drain_local_in);
void cout_drain_IO_L1_out_inter_trans(int idx, int c0, int c1, cout_t4 local_cout[14][4][14], hls::stream<cout_t4> &fifo_cout_drain_in, hls::stream<cout_t4> &fifo_cout_drain_out);
void cout_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c1, cout_t4 local_cout[14][4][14], hls::stream<cout_t4> &fifo_cout_drain_out);
              ...
              for (ap_uint<3> c11 = 0; c11 <= 3; c11 += 1) {
                ...
                #pragma HLS PIPELINE II=1
                  // simd
                  {
                    cin_t8 in_data;
                    cin_t8 out_data;
                    in_data = local_cin[c10 + 7*c6 + c8][c12 + c9][c7];
                    out_data = in_data;
                    fifo_cin_local_out.write(out_data);
                  }
                }
  ...
  for (ap_uint<5> c0 = 0; c0 <= 11; c0 += 1)
    for (ap_uint<3> c1 = 0; c1 <= 3; c1 += 1)
      for (ap_uint<4> c3 = 0; c3 <= 7; c3 += 1) {
        ...
        for (ap_uint<5> c4 = 0; c4 <= 13; c4 += 1) {
          // io_L1
          for (int c5 = 0; c5 <= min(55, -56 * c0 + 639); c5 += 1)
              ...
              for (ap_uint<3> c11 = 0; c11 <= 3; c11 += 1) {
                ...
                #pragma HLS PIPELINE II=1
                  // simd
                  {
                    w_t8 in_data;
                    w_t8 out_data;
                    in_data = local_w[c11 + 4*c5][c8][c9][c7];
                    out_data = in_data;
                    fifo_w_local_out.write(out_data);
                  }
                }
...
/* Module Definition */
void PE(int idx, hls::stream<cin_t8> &fifo_cin_in, hls::stream<float> &fifo_cout_drain_out, hls::stream<w_t8> &fifo_w_in) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  cin_t1 local_cin[1][1][8];
  #pragma HLS ARRAY_PARTITION variable=local_cin dim=0 complete
  cout_t1 local_cout[14][4][56];
              ...
              for (ap_uint<4> c8 = 0; c8 <= 6; c8 += 1) {
                ...
                for (ap_uint<3> c9 = 0; c9 <= 3; c9 += 1) {
                  ...
                  #pragma HLS PIPELINE II=1
                    // simd
                    // hls_unroll
                    local_cout[7*c6 + c8][c10][4*c5 + c9] = 0;
                  }
                }
              }
            }
                      ...
                      #pragma HLS PIPELINE II=1
                        {
                          ...
                          #pragma HLS UNROLL
                            local_cout[c10 + 7*c6][c12][c11 + 4*c5] = (local_cout[c10 + 7*c6][c12][c11 + 4*c5] + (local_cin[0][0][c13] * local_w[0][0][0][c13]));
                          }
                          if (c3 == 7 && c7 == 7 && c8 == 2 && c9 == 2)
                            fifo_cout_drain_out.write(local_cout[c10 + 7*c6][c12][c11 + 4*c5]);
                        }
                      }
                    }
                  }
        ...
        for (ap_uint<3> c11 = 0; c11 <= 3; c11 += 1) {
          ...
          #pragma HLS PIPELINE II=1
            // simd
            {
              ...
              out_data = local_cout[c10 + 7*c6][c12][(4 * c5 + c11) / 4];
              ...
              out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);              local_cout[c10 + 7*c6][c12][(4 * c5 + c11) / 4] = out_data;
            }
          }
        }
      }
