...
//  data_t cout_golden[O][R][C];
 ...
 
  {
    ...
    std::vector<float, aligned_allocator<float>> dev_cin_unserialized((58) * (58) * (512));
    std::vector<float, aligned_allocator<float>> dev_cin(10752000);
    std::vector<float, aligned_allocator<float>> dev_cout_unserialized((56) * (56) * (640));
    std::vector<float, aligned_allocator<float>> dev_cout(2007040);
    std::vector<float, aligned_allocator<float>> dev_w_unserialized((640) * (3) * (3) * (512));
    std::vector<float, aligned_allocator<float>> dev_w(41287680);

    // Initialize host buffers
    std::copy(reinterpret_cast<float *>(cin), reinterpret_cast<float *>(cin) + (58) * (58) * (512), dev_cin_unserialized.begin());
    std::copy(reinterpret_cast<float *>(w), reinterpret_cast<float *>(w) + (640) * (3) * (3) * (512), dev_w_unserialized.begin());
