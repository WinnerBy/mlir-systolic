...
typedef float w_t1;
typedef ap_uint<256> cout_t8;
typedef ap_uint<128> cout_t4;
/* Data Type */

extern "C" {
void kernel0(cin_t1 *cin, cout_t8 *cout_0, cout_t8 *cout_1, cout_t8 *cout_2, w_t1 *w);
}
void cin_IO_L1_in_intra_trans(int idx, int c0, int c1, int c3, float local_cin[16][58][1], hls::stream<float> &fifo_cin_local_out, bool intra_trans_en);
void cin_IO_L1_in_inter_trans(int idx, int c0, int c1, int c3, float local_cin[16][58][1], hls::stream<float> &fifo_cin_in, hls::stream<float> &fifo_cin_out, bool inter_trans_en);
void cin_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c3, float local_cin[16][58][1], hls::stream<float> &fifo_cin_in, bool inter_trans_en);
...
void cout_1_IO_L2_in_intra_trans(int c0, int c1, int c3, cout_t8 *cout, cout_t8 local_cout_1[14][56][7], hls::stream<float> &fifo_cout_1_local_out, bool intra_trans_en);
void w_IO_L1_in_intra_trans(int idx, int c0, int c1, int c3, float local_w[56][3][3][1], hls::stream<float> &fifo_w_local_out, bool intra_trans_en);
void w_IO_L1_in_inter_trans(int idx, int c0, int c1, int c3, float local_w[56][3][3][1], hls::stream<float> &fifo_w_in, hls::stream<float> &fifo_w_out, bool inter_trans_en);
void w_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c3, float local_w[56][3][3][1], hls::stream<float> &fifo_w_in, bool inter_trans_en);
...
void cout_1_IO_L2_out_intra_trans(int c0, int c1, int c3, cout_t8 *cout, cout_t8 local_cout_1[14][56][7], hls::stream<float> &fifo_cout_1_local_in, bool intra_trans_en);
void cout_1_IO_L2_out_inter_trans_boundary(int c0, int c1, cout_t8 *cout, cout_t8 local_cout_1[14][56][7], hls::stream<cout_t8> &fifo_cout_1_out, bool inter_trans_en);
void cout_drain_IO_L1_out_intra_trans(int idx, int c0, int c1, cout_t4 local_cout[14][56][14], hls::stream<float> &fifo_cout_drain_local_in);
void cout_drain_IO_L1_out_inter_trans(int idx, int c0, int c1, cout_t4 local_cout[14][56][14], hls::stream<cout_t4> &fifo_cout_drain_in, hls::stream<cout_t4> &fifo_cout_drain_out);
void cout_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c1, cout_t4 local_cout[14][56][14], hls::stream<cout_t4> &fifo_cout_drain_out);
          ...
          for (ap_uint<5> c5 = 0; c5 <= 15; c5 += 1)
            ...
            #pragma HLS PIPELINE II=1
              // access_coalesce
              {
                cin_t1 in_data;
                cin_t1 out_data;
                in_data = cin[415744*c1 + 64*c3 + c4 + 29696*c5 + 512*c6];
                out_data = in_data;
        ...
        for (ap_uint<3> c8 = 0; c8 <= 2; c8 += 1)
          for (ap_uint<3> c9 = 0; c9 <= 2; c9 += 1) {
            ...
            for (ap_uint<4> c10 = 0; c10 <= 6; c10 += 1) {
              // latency
              for (int c11 = 0; c11 <= min(3, -4 * c6 + 13); c11 += 1) {
                // latency
                ...
                #pragma HLS PIPELINE II=1
                  {
                    cin_t1 in_data;
                    cin_t1 out_data;
                    in_data = local_cin[c11 + 4*c6 + c8][c10 + 7*c7 + c9][0];
                    out_data = in_data;
                    fifo_cin_local_out.write(out_data);
                  }
                }
            ...
            #pragma HLS PIPELINE II=1
              {
                cout_t8 in_data;
                cout_t1 out_data;
                in_data = local_cout_1[c11 + 4*c6][c10 + 7*c7][(4 * c5 + c12) / 8];
                for (ap_uint<4> n = 0; n < 8; n++) {
                #pragma HLS UNROLL
                  data_split[n] = in_data(31, 0);
                  in_data = in_data >> 32;
                }
                int split_idx = (c12 + 4*c5) % 8;
...
/* Module Definition */
void cout_1_IO_L2_in_boundary(cout_t8 *cout, hls::stream<float> &fifo_cout_1_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  cout_t8 local_cout_1_ping[14][56][7];
  #pragma HLS RESOURCE variable=local_cout_1_ping core=RAM_2P_BRAM
  cout_t8 local_cout_1_pong[14][56][7];
            ...
            for (ap_uint<3> c6 = 0; c6 <= 2; c6 += 1)
              ...
              #pragma HLS PIPELINE II=1
                // access_coalesce
                {
                  w_t1 in_data;
                  w_t1 out_data;
                  in_data = w[258048*c0 + 64*c3 + c4 + 4608*c5 + 1536*c6 + 512*c7];
                  out_data = in_data;
                ...
                #pragma HLS PIPELINE II=1
                  {
                    w_t1 in_data;
                    w_t1 out_data;
                    in_data = local_w[c12 + 4*c5][c8][c9][0];
                    out_data = in_data;
                    fifo_w_local_out.write(out_data);
                  }
                }
            ...
            for (ap_uint<4> c7 = 0; c7 <= 7; c7 += 1) {
              if (p0 == 0 && c3 == 0) {
                ...
                for (ap_uint<4> c8 = 0; c8 <= 6; c8 += 1) {
                  // latency
                  for (int c9 = 0; c9 <= min(3, -4 * c6 + 13); c9 += 1) {
                    // latency
                    for (ap_uint<3> c10 = 0; c10 <= 3; c10 += 1) {
                    #pragma HLS PIPELINE II=1
                      local_cout[4*c6 + c9][7*c7 + c8][c10 + 4*c5] = 0;
                    }
                  }
                }
              }
                      ...
                      #pragma HLS PIPELINE II=1
                        {
                          ...
                          if (p0 + 64 * c3 >= 1 && c8 == 0 && c9 == 0)
                            local_cout[c11 + 4*c6][c10 + 7*c7][c12 + 4*c5] = fifo_cout_1_in.read();
                          local_w[0][0][0][0] = fifo_w_in.read();
                          local_cout[c11 + 4*c6][c10 + 7*c7][c12 + 4*c5] = (local_cout[c11 + 4*c6][c10 + 7*c7][c12 + 4*c5] + (local_cin[0][0][0] * local_w[0][0][0][0]));
                          if (p0 == 63 && c3 == 7 && c8 == 2 && c9 == 2) {
                            fifo_cout_drain_out.write(local_cout[c11 + 4*c6][c10 + 7*c7][c12 + 4*c5]);
                          } else if (p0 + 64 * c3 <= 510 && c8 == 2 && c9 == 2) {
                            fifo_cout_1_out.write(local_cout[c11 + 4*c6][c10 + 7*c7][c12 + 4*c5]);
                          }
                        }
                      }
                    }
            ...
            #pragma HLS PIPELINE II=1
              {
                ...
                int split_idx = (c12 + 4*c5) % 8;
                out_data = local_cout_1[c11 + 4*c6][c10 + 7*c7][(4 * c5 + c12) / 8];
                for (ap_uint<4> n = 0; n < 8; n++) {
                #pragma HLS UNROLL
                ...
                out_data = (data_split[7], data_split[6], data_split[5], data_split[4], data_split[3], data_split[2], data_split[1], data_split[0]);                local_cout_1[c11 + 4*c6][c10 + 7*c7][(4 * c5 + c12) / 8] = out_data;
              }
            }
          }
        }
...
#pragma HLS INLINE OFF
  ...
  for (ap_uint<5> c4 = 0; c4 <= 13; c4 += 1)
    for (ap_uint<7> c5 = 0; c5 <= 55; c5 += 1) {
      // access_coalesce
      for (int c6 = 0; c6 <= min(6, -7 * c0 + 79); c6 += 1) {
      #pragma HLS PIPELINE II=1
        {
          cout_t8 in_data;
          cout_t8 out_data;
          ...
          out_data = in_data;
          cout[7*c0 + 62720*c1 + 4480*c4 + 80*c5 + c6] = out_data;
        }
      }
    }
}
...
/* Module Definition */
void cout_1_IO_L2_out_boundary(cout_t8 *cout, hls::stream<float> &fifo_cout_1_local_in) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  cout_t8 local_cout_1_ping[14][56][7];
  #pragma HLS RESOURCE variable=local_cout_1_ping core=RAM_2P_BRAM
            ...
            #pragma HLS PIPELINE II=1
              {
                ...
                int split_idx = (c12 + 4*c5) % 4;
                out_data = local_cout[c11 + 4*c6][c10 + 7*c7][(4 * c5 + c12) / 4];
                ...
                u.ut = in_data;
                data_split[split_idx] = ap_uint<32>(u.ui);
                out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);                local_cout[c11 + 4*c6][c10 + 7*c7][(4 * c5 + c12) / 4] = out_data;
              }
            }
          }
        }
...
/* Module Definition */
void cout_drain_IO_L2_out(cout_t8 *cout, hls::stream<cout_t4> &fifo_cout_drain_local_in) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  cout_t4 data_split[2];
  #pragma HLS ARRAY_PARTITION variable=data_split complete
  /* Variable Declaration */
          ...
          #pragma HLS PIPELINE II=1
            {
              ...
              cout_t8 out_data;
              in_data = fifo_cout_drain_local_in.read();
              int split_idx = (14*c0 + 125440*c1 + 8960*c5 + 160*c6 + c7) % 2;
              data_split[split_idx] = in_data;
              out_data = (data_split[1], data_split[0]);              if (c7 % 2 == 2 - 1 || c7 == 13) {
                cout[(((14 * c1 + c5) * 56 + c6) * 640 + (56 * c0 + 4 * c7)) / 8] = out_data;
              }
            }
          }
        }
