...
void A_IO_L1_in_intra_trans(int idx, int idy, int c0, int c1, int c2, int c3, A_t8 local_A[1][8][4], hls::stream<A_t8> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L1_in_inter_trans(int idx, int idy, int c0, int c1, int c2, int c3, A_t8 local_A[1][8][4], hls::stream<A_t8> &fifo_A_in, hls::stream<A_t8> &fifo_A_out, bool inter_trans_en);
void A_IO_L1_in_inter_trans_boundary(int idx, int idy, int c0, int c1, int c2, int c3, A_t8 local_A[1][8][4], hls::stream<A_t8> &fifo_A_in, bool inter_trans_en);
...
void B_IO_L2_in_intra_trans(int idx, int c0, int c1, int c2, int c3, B_t16 local_B[16][2], hls::stream<B_t8> &fifo_B_local_out, bool intra_trans_en);
void B_IO_L2_in_inter_trans(int idx, int c0, int c1, int c2, int c3, B_t16 local_B[16][2], hls::stream<B_t16> &fifo_B_in, hls::stream<B_t16> &fifo_B_out, bool inter_trans_en);
void B_IO_L2_in_inter_trans_boundary(int idx, int c0, int c1, int c2, int c3, B_t16 local_B[16][2], hls::stream<B_t16> &fifo_B_in, bool inter_trans_en);
void PE_wrapper(int idx, int idy, hls::stream<A_t8> &fifo_A_in, hls::stream<B_t8> &fifo_B_in, hls::stream<B_t8> &fifo_B_out, hls::stream<float> &fifo_C_drain_out);
void C_drain_IO_L1_out_intra_trans(int idx, int idy, int c0, int c1, int c2, C_t4 local_C[1][8][4], hls::stream<float> &fifo_C_drain_local_in);
void C_drain_IO_L1_out_inter_trans(int idx, int idy, int c0, int c1, int c2, C_t4 local_C[1][8][4], hls::stream<C_t4> &fifo_C_drain_in, hls::stream<C_t4> &fifo_C_drain_out);
void C_drain_IO_L1_out_inter_trans_boundary(int idx, int idy, int c0, int c1, int c2, C_t4 local_C[1][8][4], hls::stream<C_t4> &fifo_C_drain_out);
      ...
      for (ap_uint<5> c2 = 0; c2 <= 15; c2 += 1)
        for (ap_uint<4> c3 = 0; c3 <= 7; c3 += 1) {
          ...
          for (ap_uint<4> c4 = 0; c4 <= 7; c4 += 1) {
            // io_L2
            for (int c5 = 0; c5 <= min(15, -16 * c0 + 263); c5 += 1) {
              // io_L1
...
#pragma HLS INLINE OFF
  ...
  A_t8 fifo_data;
  A_t16 mem_data;
  for (ap_uint<26> i = 0; i < 17301504; i++) {
  #pragma HLS PIPELINE II=1
    mem_data = A[i];
...
#pragma HLS INLINE OFF
  ...
  for (int c5 = p1; c5 <= min(15, -16 * c0 + 263); c5 += 1) {
    // io_L1
    if (c5 == p1) {
      for (ap_uint<4> c7 = 0; c7 <= 7; c7 += 1) {
        // access_coalesce
...
#pragma HLS INLINE OFF
  ...
  for (ap_uint<23> i = 0; i < 2228224; i++) {
  #pragma HLS PIPELINE II=1
    B_t16 fifo_data;
    fifo_data = B[i];
      ...
      for (ap_uint<4> c8 = 0; c8 <= 7; c8 += 1) {
        ...
        #pragma HLS PIPELINE II=1
          // simd
          {
            B_t16 in_data;
            B_t8 out_data;
            in_data = local_B[8*c6 + c8][8 * c7 / 16];
...
/* Module Definition */
void PE(int idx, int idy, hls::stream<A_t8> &fifo_A_in, hls::stream<B_t8> &fifo_B_in, hls::stream<B_t8> &fifo_B_out, hls::stream<float> &fifo_C_drain_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx, p1 = idy; // module id
  A_t1 local_A[1][1][8];
                  ...
                  #pragma HLS PIPELINE II=1
                    {
                      ...
                      #pragma HLS UNROLL
                        local_C[0][c9][8*c6 + c8] = (local_C[0][c9][8*c6 + c8] + (local_A[0][0][c10] * local_B[0][c10]));
                      }
                      if (c3 == 7 && c7 == 3)
                        fifo_C_drain_out.write(local_C[0][c9][8*c6 + c8]);
                      {
                        B_t8 fifo_data;
    ...
    for (ap_uint<4> c8 = 0; c8 <= 7; c8 += 1) {
      ...
      #pragma HLS PIPELINE II=1
        // simd
        {
          ...
          int split_idx = (8*c6 + c8) % 4;
          out_data = local_C[0][c9][(8 * c6 + c8) / 4];
          ...
          out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);          local_C[0][c9][(8 * c6 + c8) / 4] = out_data;
        }
      }
    }
  }
...
#pragma HLS INLINE OFF
  ...
  for (ap_uint<22> i = 0; i < 1081344; i++) {
  #pragma HLS PIPELINE II=1
    C_t4 fifo_data;
    C_t16 mem_data;
