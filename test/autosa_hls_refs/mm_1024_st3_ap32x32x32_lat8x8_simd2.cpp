...
void A_IO_L2_in_intra_trans(int idx, int c0, int c1, int c2, A_t16 local_A[8][2], hls::stream<A_t2> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L2_in_inter_trans(int idx, int c0, int c1, int c2, A_t16 local_A[8][2], hls::stream<A_t16> &fifo_A_in, hls::stream<A_t16> &fifo_A_out, bool inter_trans_en);
void A_IO_L2_in_inter_trans_boundary(int idx, int c0, int c1, int c2, A_t16 local_A[8][2], hls::stream<A_t16> &fifo_A_in, bool inter_trans_en);
            ...
            #pragma HLS PIPELINE II=1
              {
                A_t16 in_data;
                A_t16 out_data;
                in_data = A[128*c0 + 2*c2 + 32*c3 + 4*c4 + c5];
                out_data = in_data;
            ...
            #pragma HLS PIPELINE II=1
              {
                ...
                C_t8 out_data;
                in_data = fifo_C_drain_local_in.read();
                int split_idx = (512*c0 + 8*c1 + 2*c3 + 128*c4 + 16*c5 + c6) % 2;
                data_split[split_idx] = in_data;
                out_data = (data_split[1], data_split[0]);                if (c6 % 2 == 2 - 1 || c6 == 1) {
                  C[((32 * c0 + 8 * c4 + c5) * 64 + (32 * c1 + 8 * c3 + 4 * c6)) / 8] = out_data;
                }
              }
            }
          }
