...
void A_IO_L2_in_intra_trans(int c0, int c1, int c2, A_t16 *A, A_t16 local_A[32][2], hls::stream<A_t2> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L2_in_inter_trans_boundary(int c0, int c1, int c2, A_t16 *A, A_t16 local_A[32][2], hls::stream<A_t16> &fifo_A_in, bool inter_trans_en);
void B_IO_L1_in_intra_trans(int idx, int c0, int c1, int c2, B_t4 local_B[8][8], hls::stream<B_t2> &fifo_B_local_out, bool intra_trans_en);
void B_IO_L1_in_inter_trans(int idx, int c0, int c1, int c2, B_t4 local_B[8][8], hls::stream<B_t4> &fifo_B_in, hls::stream<B_t4> &fifo_B_out, bool inter_trans_en);
void B_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c2, B_t4 local_B[8][8], hls::stream<B_t4> &fifo_B_in, bool inter_trans_en);
void B_IO_L1_in_wrapper(int idx, hls::stream<B_t4> &fifo_B_in, hls::stream<B_t4> &fifo_B_out, hls::stream<B_t2> &fifo_B_local_out);
void B_IO_L1_in_boundary_wrapper(int idx, hls::stream<B_t4> &fifo_B_in, hls::stream<B_t2> &fifo_B_local_out);
void PE_wrapper(int idx, hls::stream<A_t2> &fifo_A_in, hls::stream<A_t2> &fifo_A_out, hls::stream<B_t2> &fifo_B_in, hls::stream<float> &fifo_C_drain_out);
void C_drain_IO_L1_out_intra_trans(int idx, int c0, int c1, C_t4 local_C[32][2], hls::stream<float> &fifo_C_drain_local_in);
void C_drain_IO_L1_out_inter_trans(int idx, int c0, int c1, C_t4 local_C[32][2], hls::stream<C_t4> &fifo_C_drain_in, hls::stream<C_t4> &fifo_C_drain_out);
void C_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c1, C_t4 local_C[32][2], hls::stream<C_t4> &fifo_C_drain_out);
      ...
      for (ap_uint<4> c6 = 0; c6 <= 7; c6 += 1) {
        ...
        #pragma HLS PIPELINE II=1
          // simd
          {
            A_t16 in_data;
            A_t2 out_data;
            in_data = local_A[8*c4 + c6][2 * c5 / 16];
    ...
    #pragma HLS PIPELINE II=1
      {
        A_t16 in_data;
        A_t16 out_data;
        in_data = A[128*c0 + 2*c2 + 4*c3 + c4];
        out_data = in_data;
        local_A[c3][c4] = out_data;
      }
    }
  }
}
...
/* Module Definition */
void A_IO_L2_in_boundary(A_t16 *A, hls::stream<A_t2> &fifo_A_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  A_t16 local_A_ping[32][2];
  #pragma HLS RESOURCE variable=local_A_ping core=RAM_2P_BRAM
  A_t16 local_A_pong[32][2];
            ...
            #pragma HLS PIPELINE II=1
              {
                ...
                B_t4 out_data;
                if (c5 % 4 == 0) {
                  in_data = B[((32 * c1 + 8 * c3 + c4) * 64 + (32 * c2 + 4 * c5)) / 16];
                ...
                int split_idx = (512*c1 + 8*c2 + 128*c3 + 16*c4 + c5) % 4;
                out_data = data_split[split_idx];
...
/* Module Definition */
void B_IO_L1_in(int idx, hls::stream<B_t4> &fifo_B_in, hls::stream<B_t4> &fifo_B_out, hls::stream<B_t2> &fifo_B_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  B_t4 local_B_ping[8][8];
  #pragma HLS RESOURCE variable=local_B_ping core=RAM_2P_BRAM
  B_t4 local_B_pong[8][8];
...
/* Module Definition */
void B_IO_L1_in_boundary(int idx, hls::stream<B_t4> &fifo_B_in, hls::stream<B_t2> &fifo_B_local_out) {
#pragma HLS INLINE
  /* Variable Declaration */
  int p0 = idx; // module id
  B_t4 local_B_ping[8][8];
...
/* Module Definition */
void PE(int idx, hls::stream<A_t2> &fifo_A_in, hls::stream<A_t2> &fifo_A_out, hls::stream<B_t2> &fifo_B_in, hls::stream<float> &fifo_C_drain_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  A_t1 local_A[1][2];
              ...
              #pragma HLS PIPELINE II=1
                {
                  ...
                  #pragma HLS UNROLL
                    local_C[8*c4 + c6][c7] = (local_C[8*c4 + c6][c7] + (local_A[0][c8] * local_B[0][c8]));
                  }
                  if (c2 == 1 && c5 == 15)
                    fifo_C_drain_out.write(local_C[8*c4 + c6][c7]);
                  {
                    A_t2 fifo_data;
    ...
    for (ap_uint<4> c6 = 0; c6 <= 7; c6 += 1) {
      ...
      #pragma HLS PIPELINE II=1
        // simd
        {
          ...
          out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);          local_C[8*c4 + c6][c7 / 4] = out_data;
        }
      }
    }
  }
...
/* Module Definition */
void C_drain_IO_L2_out(C_t8 *C, hls::stream<C_t4> &fifo_C_drain_local_in) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  C_t4 data_split[2];
  #pragma HLS ARRAY_PARTITION variable=data_split complete
          ...
          #pragma HLS PIPELINE II=1
            {
              ...
              C_t8 out_data;
              in_data = fifo_C_drain_local_in.read();
              int split_idx = (512*c0 + 8*c1 + 2*c3 + 16*c4 + c5) % 2;
              data_split[split_idx] = in_data;
              out_data = (data_split[1], data_split[0]);              if (c5 % 2 == 2 - 1 || c5 == 1) {
                C[((32 * c0 + c4) * 64 + (32 * c1 + 8 * c3 + 4 * c5)) / 8] = out_data;
              }
            }
          }
        }
