            ...
            #pragma HLS PIPELINE II=1
              {
                A_t16 in_data;
                A_t16 out_data;
                in_data = A[64*c2 + 131072*c3 + 4096*c4 + 16*c5 + c6];
                out_data = in_data;
      ...
      for (int c3 = 0; c3 <= min(15, -16 * c1 + 20); c3 += 1) {
        ...
        #pragma HLS PIPELINE II=1
          // access_coalesce
          {
            B_t16 in_data;
            B_t16 out_data;
            in_data = B[16*c1 + 84*c2 + c3 + 21*c4];
            out_data = in_data;
        ...
        #pragma HLS PIPELINE II=1
          {
            C_t16 in_data;
            C_t16 out_data;
            in_data = C[4096*c1 + 256*c3 + 16*c4 + c5];
            out_data = in_data;
          ...
          #pragma HLS PIPELINE II=1
            {
              ...
              D_t16 out_data;
              in_data = fifo_D_drain_local_in.read();
              int split_idx = (64*c1 + 4*c3 + 2688*c4 + 84*c5 + c6) % 4;
              data_split[split_idx] = in_data;
              out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);              if (c6 % 4 == 4 - 1 || c6 == 3) {
                D[((32 * c4 + c5) * 336 + (256 * c1 + 16 * c3 + 4 * c6)) / 16] = out_data;
              }
            }
          }
        }
