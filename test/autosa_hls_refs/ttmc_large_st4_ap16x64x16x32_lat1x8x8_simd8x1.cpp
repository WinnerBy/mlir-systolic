...
extern "C" {
void kernel0(A_t16 *A, B_t8 *B, C_t16 *C, D_t16 *D);
}
              ...
              #pragma HLS PIPELINE II=1
                {
                  A_t16 in_data;
                  A_t16 out_data;
                  in_data = A[16384*c0 + 256*c3 + 1024*c4 + 8*c6 + c7];
                  out_data = in_data;
          ...
          for (ap_uint<4> c4 = 0; c4 <= 7; c4 += 1) {
            ...
            #pragma HLS PIPELINE II=1
              // access_coalesce
              {
                B_t8 in_data;
                B_t8 out_data;
                in_data = B[8*c1 + 512*c3 + c4 + 16*c5];
                out_data = in_data;
              ...
              #pragma HLS PIPELINE II=1
                {
                  ...
                  D_t16 out_data;
                  in_data = fifo_D_drain_local_in.read();
                  int split_idx = (65536*c0 + 2048*c1 + 4*c2 + 256*c4 + 4096*c5 + 32*c7 + c8) % 4;
                  data_split[split_idx] = in_data;
                  out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);                  if (c8 % 4 == 4 - 1 || c8 == 3) {
                    D[(((16 * c0 + c5) * 128 + (64 * c1 + 8 * c4 + c7)) * 128 + (16 * c2 + 4 * c8)) / 16] = out_data;
                  }
                }
              }
            }
