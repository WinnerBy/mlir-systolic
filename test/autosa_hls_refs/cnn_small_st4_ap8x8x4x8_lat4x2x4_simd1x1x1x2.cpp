          ...
          for (ap_uint<2> c11 = 0; c11 <= 1; c11 += 1) {
            ...
            #pragma HLS PIPELINE II=1
              // simd
              {
                cin_t8 in_data;
                cin_t2 out_data;
                in_data = local_cin[c11 + c8][c12 + c9][2 * c7 / 8];
          ...
          for (ap_uint<2> c11 = 0; c11 <= 1; c11 += 1) {
            ...
            #pragma HLS PIPELINE II=1
              // simd
              {
                w_t8 in_data;
                w_t2 out_data;
                in_data = local_w[c10][c8][c9][2 * c7 / 8];
                for (ap_uint<3> n = 0; n < 4; n++) {
                #pragma HLS UNROLL
                  data_split[n] = in_data(63, 0);
                  in_data = in_data >> 64;
                    ...
                    #pragma HLS PIPELINE II=1
                      {
                        ...
                        #pragma HLS UNROLL
                          local_cout[c11][c12][c10] = (local_cout[c11][c12][c10] + (local_cin[0][0][c13] * local_w[0][0][0][c13]));
                        }
                        {
                          w_t2 fifo_data;
