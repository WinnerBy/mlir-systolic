...
void A_IO_L2_in_intra_trans(int idx, int c0, int c1, int c2, A_t16 local_A[8][1], hls::stream<A_t4> &fifo_A_local_out, bool intra_trans_en);
...
void B_IO_L2_in_intra_trans(int idx, int c0, int c1, int c2, B_t16 local_B[8][1], hls::stream<B_t4> &fifo_B_local_out, bool intra_trans_en);
            ...
            #pragma HLS PIPELINE II=1
              {
                ...
                C_t8 out_data;
                in_data = fifo_C_drain_local_in.read();
                int split_idx = (256*c0 + 4*c1 + 2*c3 + 128*c4 + 16*c5 + c6) % 2;
                data_split[split_idx] = in_data;
                out_data = (data_split[1], data_split[0]);                if (c6 % 2 == 2 - 1 || c6 == 1) {
                  C[((16 * c0 + 8 * c4 + c5) * 64 + (16 * c1 + 8 * c3 + 4 * c6)) / 8] = out_data;
                }
              }
            }
          }
