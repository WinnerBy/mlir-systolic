...
void A_IO_L1_in_intra_trans(int idx, int c0, int c1, int c2, A_t4 local_A[8][8], hls::stream<A_t2> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L1_in_inter_trans(int idx, int c0, int c1, int c2, A_t4 local_A[8][8], hls::stream<A_t4> &fifo_A_in, hls::stream<A_t4> &fifo_A_out, bool inter_trans_en);
void A_IO_L1_in_inter_trans_boundary(int idx, int c0, int c1, int c2, A_t4 local_A[8][8], hls::stream<A_t4> &fifo_A_in, bool inter_trans_en);
...
void B_IO_L2_in_intra_trans(int c0, int c1, int c2, B_t16 *B, B_t16 local_B[32][2], hls::stream<B_t2> &fifo_B_local_out, bool intra_trans_en);
void B_IO_L2_in_inter_trans_boundary(int c0, int c1, int c2, B_t16 *B, B_t16 local_B[32][2], hls::stream<B_t16> &fifo_B_in, bool inter_trans_en);
...
void C_drain_IO_L1_out_intra_trans(int idx, int c0, int c1, C_t4 local_C[8][8], hls::stream<float> &fifo_C_drain_local_in);
void C_drain_IO_L1_out_inter_trans(int idx, int c0, int c1, C_t4 local_C[8][8], hls::stream<C_t4> &fifo_C_drain_in, hls::stream<C_t4> &fifo_C_drain_out);
void C_drain_IO_L1_out_inter_trans_boundary(int idx, int c0, int c1, C_t4 local_C[8][8], hls::stream<C_t4> &fifo_C_drain_out);
            ...
            #pragma HLS PIPELINE II=1
              {
                ...
                A_t4 out_data;
                if (c5 % 4 == 0) {
                  in_data = A[((32 * c0 + 8 * c3 + c4) * 64 + (32 * c2 + 4 * c5)) / 16];
                ...
                int split_idx = (512*c0 + 8*c2 + 128*c3 + 16*c4 + c5) % 4;
...
/* Module Definition */
void B_IO_L2_in_boundary(B_t16 *B, hls::stream<B_t2> &fifo_B_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
...
/* Module Definition */
void C_drain_IO_L2_out(C_t16 *C, hls::stream<C_t4> &fifo_C_drain_local_in) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  C_t4 data_split[4];
  #pragma HLS ARRAY_PARTITION variable=data_split complete
          ...
          #pragma HLS PIPELINE II=1
            {
              ...
              C_t16 out_data;
              in_data = fifo_C_drain_local_in.read();
              int split_idx = (512*c0 + 8*c1 + 128*c3 + 16*c4 + c5) % 4;
              data_split[split_idx] = in_data;
              out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);              if (c5 % 4 == 4 - 1 || c5 == 7) {
                C[((32 * c0 + 8 * c3 + c4) * 64 + (32 * c1 + 4 * c5)) / 16] = out_data;
              }
            }
          }
        }
