...
void A_IO_L3_in(hls::stream<A_t8> &fifo_A_in, hls::stream<A_t8> &fifo_A_local_out) {
#pragma HLS INLINE OFF
  for (ap_uint<3> c0 = 0; c0 <= 1; c0 += 1)
    for (ap_uint<3> c1 = 0; c1 <= 1; c1 += 1)
      for (ap_uint<3> c2 = 0; c2 <= 1; c2 += 1) {
        for (ap_uint<2> c3 = 0; c3 <= 1; c3 += 1) {
          for (ap_uint<3> c4 = 0; c4 <= 3; c4 += 1) {
          #pragma HLS PIPELINE II=1
            A_t8 in_data = fifo_A_in.read();
            fifo_A_local_out.write(in_data);
          }
        }
      }
}
    ...
    for (ap_uint<3> c1 = 0; c1 <= 1; c1 += 1)
      for (ap_uint<3> c2 = 0; c2 <= 1; c2 += 1) {
        for (ap_uint<2> c3 = 0; c3 <= 1; c3 += 1) {
          for (ap_uint<3> c4 = 0; c4 <= 3; c4 += 1) {
          #pragma HLS PIPELINE II=1
            B_t8 in_data = fifo_B_in.read();
            fifo_B_local_out.write(in_data);
          }
        }
      }
}
