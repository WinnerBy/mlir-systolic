...
void A_IO_L2_in_intra_trans(int idx, int c0, int c1, int c2, A_t16 local_A[2][2], hls::stream<A_t2> &fifo_A_local_out, bool intra_trans_en);
void A_IO_L2_in_inter_trans(int idx, int c0, int c1, int c2, A_t16 local_A[2][2], hls::stream<A_t16> &fifo_A_in, hls::stream<A_t16> &fifo_A_out, bool inter_trans_en);
void A_IO_L2_in_inter_trans_boundary(int idx, int c0, int c1, int c2, A_t16 local_A[2][2], hls::stream<A_t16> &fifo_A_in, bool inter_trans_en);
void B_IO_L2_in_intra_trans(int idx, int c0, int c1, int c2, B_t16 local_B[8][2], hls::stream<B_t2> &fifo_B_local_out, bool intra_trans_en);
void B_IO_L2_in_inter_trans(int idx, int c0, int c1, int c2, B_t16 local_B[8][2], hls::stream<B_t16> &fifo_B_in, hls::stream<B_t16> &fifo_B_out, bool inter_trans_en);
void B_IO_L2_in_inter_trans_boundary(int idx, int c0, int c1, int c2, B_t16 local_B[8][2], hls::stream<B_t16> &fifo_B_in, bool inter_trans_en);
...
void C_drain_IO_L1_out_intra_trans(int idx, int idy, int c0, int c1, C_t4 local_C[2][2], hls::stream<float> &fifo_C_drain_local_in);
void C_drain_IO_L1_out_inter_trans(int idx, int idy, int c0, int c1, C_t4 local_C[2][2], hls::stream<C_t4> &fifo_C_drain_in, hls::stream<C_t4> &fifo_C_drain_out);
void C_drain_IO_L1_out_inter_trans_boundary(int idx, int idy, int c0, int c1, C_t4 local_C[2][2], hls::stream<C_t4> &fifo_C_drain_out);
            ...
            #pragma HLS PIPELINE II=1
              {
                A_t16 in_data;
                A_t16 out_data;
                in_data = A[16*c0 + 2*c2 + 8*c3 + 4*c4 + c5];
                out_data = in_data;
            ...
            #pragma HLS PIPELINE II=1
              {
                B_t16 in_data;
                B_t16 out_data;
                in_data = B[128*c1 + 2*c2 + 32*c3 + 4*c4 + c5];
                out_data = in_data;
            ...
            #pragma HLS PIPELINE II=1
              {
                ...
                C_t8 out_data;
                in_data = fifo_C_drain_local_in.read();
                int split_idx = (64*c0 + 8*c1 + 2*c3 + 32*c4 + 16*c5 + c6) % 2;
                data_split[split_idx] = in_data;
                out_data = (data_split[1], data_split[0]);                if (c6 % 2 == 2 - 1 || c6 == 1) {
                  C[((4 * c0 + 2 * c4 + c5) * 64 + (32 * c1 + 8 * c3 + 4 * c6)) / 8] = out_data;
                }
              }
            }
          }
