...
typedef ap_uint<128> w_t4;
/* Data Type */

extern "C" {
void kernel0(cin_t16 *cin, cout_t16 *cout, w_t16 *w);
}
void cin_IO_L2_in_intra_trans(int idx, int c0, int c1, int c2, int c3, cin_t8 local_cin[10][4][1], hls::stream<float> &fifo_cin_local_out, bool intra_trans_en);
void cin_IO_L2_in_inter_trans(int idx, int c0, int c1, int c2, int c3, cin_t8 local_cin[10][4][1], hls::stream<cin_t8> &fifo_cin_in, hls::stream<cin_t8> &fifo_cin_out, bool inter_trans_en);
void cin_IO_L2_in_inter_trans_boundary(int idx, int c0, int c1, int c2, int c3, cin_t8 local_cin[10][4][1], hls::stream<cin_t8> &fifo_cin_in, bool inter_trans_en);
void w_IO_L1_in_intra_trans(int idx, int idy, int c0, int c1, int c2, int c3, w_t4 local_w[4][3][3][2], hls::stream<float> &fifo_w_local_out, bool intra_trans_en);
void w_IO_L1_in_inter_trans(int idx, int idy, int c0, int c1, int c2, int c3, w_t4 local_w[4][3][3][2], hls::stream<w_t4> &fifo_w_in, hls::stream<w_t4> &fifo_w_out, bool inter_trans_en);
void w_IO_L1_in_inter_trans_boundary(int idx, int idy, int c0, int c1, int c2, int c3, w_t4 local_w[4][3][3][2], hls::stream<w_t4> &fifo_w_in, bool inter_trans_en);
void w_IO_L1_in_wrapper(int idx, int idy, hls::stream<w_t4> &fifo_w_in, hls::stream<w_t4> &fifo_w_out, hls::stream<float> &fifo_w_local_out);
void w_IO_L1_in_boundary_wrapper(int idx, int idy, hls::stream<w_t4> &fifo_w_in, hls::stream<float> &fifo_w_local_out);
void PE_wrapper(int idx, int idy, hls::stream<float> &fifo_cin_in, hls::stream<float> &fifo_cin_out, hls::stream<float> &fifo_cout_drain_out, hls::stream<float> &fifo_w_in);
void cout_drain_IO_L1_out_intra_trans(int idx, int idy, int c0, int c1, int c2, cout_t4 local_cout[8][2][1], hls::stream<float> &fifo_cout_drain_local_in);
void cout_drain_IO_L1_out_inter_trans(int idx, int idy, int c0, int c1, int c2, cout_t4 local_cout[8][2][1], hls::stream<cout_t4> &fifo_cout_drain_in, hls::stream<cout_t4> &fifo_cout_drain_out);
void cout_drain_IO_L1_out_inter_trans_boundary(int idx, int idy, int c0, int c1, int c2, cout_t4 local_cout[8][2][1], hls::stream<cout_t4> &fifo_cout_drain_out);
              ...
              #pragma HLS PIPELINE II=1
                {
                  cin_t8 in_data;
                  cin_t1 out_data;
                  in_data = local_cin[c12 + 4*c6 + c8][c11 + c9][c7 / 8];
...
/* Module Definition */
void w_IO_L3_in(hls::stream<w_t4> &fifo_w_in, hls::stream<w_t4> &fifo_w_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  /* Variable Declaration */
...
/* Module Definition */
void w_IO_L3_in_serialize(w_t16 *w, hls::stream<w_t4> &fifo_w_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  /* Variable Declaration */
...
/* Module Definition */
void w_IO_L2_in(int idx, hls::stream<w_t4> &fifo_w_in, hls::stream<w_t4> &fifo_w_out, hls::stream<w_t4> &fifo_w_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  /* Variable Declaration */
...
/* Module Definition */
void w_IO_L2_in_boundary(int idx, hls::stream<w_t4> &fifo_w_in, hls::stream<w_t4> &fifo_w_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  /* Variable Declaration */
              ...
              #pragma HLS PIPELINE II=1
                {
                  w_t4 in_data;
                  w_t1 out_data;
                  in_data = local_w[c10][c8][c9][c7 / 4];
...
/* Module Definition */
void w_IO_L1_in(int idx, int idy, hls::stream<w_t4> &fifo_w_in, hls::stream<w_t4> &fifo_w_out, hls::stream<float> &fifo_w_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx, p1 = idy; // module id
  w_t4 local_w_ping[4][3][3][2];
  #pragma HLS RESOURCE variable=local_w_ping core=RAM_2P_BRAM
  w_t4 local_w_pong[4][3][3][2];
...
/* Module Definition */
void w_IO_L1_in_boundary(int idx, int idy, hls::stream<w_t4> &fifo_w_in, hls::stream<float> &fifo_w_local_out) {
#pragma HLS INLINE
  /* Variable Declaration */
  int p0 = idx, p1 = idy; // module id
  w_t4 local_w_ping[4][3][3][2];
...
/* Module Definition */
void PE(int idx, int idy, hls::stream<float> &fifo_cin_in, hls::stream<float> &fifo_cin_out, hls::stream<float> &fifo_cout_drain_out, hls::stream<float> &fifo_w_in) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx, p1 = idy; // module id
  cin_t1 local_cin[1][1][1];
  #pragma HLS ARRAY_PARTITION variable=local_cin dim=0 complete
  cout_t1 local_cout[8][2][4];
                      ...
                      #pragma HLS PIPELINE II=1
                        {
                          ...
                          local_cout[c12 + 4*c6][c11][c10] = (local_cout[c12 + 4*c6][c11][c10] + (local_cin[0][0][0] * local_w[0][0][0][0]));
                          if (c3 == 1 && c7 == 7 && c8 == 2 && c9 == 2)
                            fifo_cout_drain_out.write(local_cout[c12 + 4*c6][c11][c10]);
        ...
        #pragma HLS PIPELINE II=1
          {
            ...
            int split_idx = (c10) % 4;
            out_data = local_cout[c12 + 4*c6][c11][c10 / 4];
            for (ap_uint<3> n = 0; n < 4; n++) {
            #pragma HLS UNROLL
            ...
            out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);            local_cout[c12 + 4*c6][c11][c10 / 4] = out_data;
          }
        }
      }
    }
...
#pragma HLS DATAFLOW
  ...
  /* w_IO_L3_in_serialize fifo */ hls::stream<w_t4> fifo_w_w_IO_L3_in_serialize;
