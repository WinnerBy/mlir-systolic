...
void cin_IO_L2_in_intra_trans(int idx, int c0, int c1, int c2, int c3, cin_t8 local_cin[4][6][1], hls::stream<cin_t2> &fifo_cin_local_out, bool intra_trans_en);
void cin_IO_L2_in_inter_trans(int idx, int c0, int c1, int c2, int c3, cin_t8 local_cin[4][6][1], hls::stream<cin_t8> &fifo_cin_in, hls::stream<cin_t8> &fifo_cin_out, bool inter_trans_en);
void cin_IO_L2_in_inter_trans_boundary(int idx, int c0, int c1, int c2, int c3, cin_t8 local_cin[4][6][1], hls::stream<cin_t8> &fifo_cin_in, bool inter_trans_en);
...
void PE_wrapper(int idx, int idy, hls::stream<cin_t2> &fifo_cin_in, hls::stream<cin_t2> &fifo_cin_out, hls::stream<float> &fifo_cout_drain_out, hls::stream<w_t2> &fifo_w_in, hls::stream<w_t2> &fifo_w_out);
void cout_drain_IO_L1_out_intra_trans(int idx, int idy, int c0, int c1, int c2, cout_t4 local_cout[2][4][1], hls::stream<float> &fifo_cout_drain_local_in);
void cout_drain_IO_L1_out_inter_trans(int idx, int idy, int c0, int c1, int c2, cout_t4 local_cout[2][4][1], hls::stream<cout_t4> &fifo_cout_drain_in, hls::stream<cout_t4> &fifo_cout_drain_out);
void cout_drain_IO_L1_out_inter_trans_boundary(int idx, int idy, int c0, int c1, int c2, cout_t4 local_cout[2][4][1], hls::stream<cout_t4> &fifo_cout_drain_out);
          ...
          for (ap_uint<2> c11 = 0; c11 <= 1; c11 += 1) {
            ...
            #pragma HLS PIPELINE II=1
              // simd
              {
                cin_t8 in_data;
                cin_t2 out_data;
                in_data = local_cin[c11 + c8][c10 + c9][2 * c7 / 8];
...
/* Module Definition */
void cin_IO_L2_in(int idx, hls::stream<cin_t8> &fifo_cin_in, hls::stream<cin_t8> &fifo_cin_out, hls::stream<cin_t2> &fifo_cin_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
...
/* Module Definition */
void cin_IO_L2_in_boundary(int idx, hls::stream<cin_t8> &fifo_cin_in, hls::stream<cin_t2> &fifo_cin_local_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
  int p0 = idx; // module id
  cin_t8 local_cin_ping[4][6][1];
...
/* Module Definition */
void PE(int idx, int idy, hls::stream<cin_t2> &fifo_cin_in, hls::stream<cin_t2> &fifo_cin_out, hls::stream<float> &fifo_cout_drain_out, hls::stream<w_t2> &fifo_w_in, hls::stream<w_t2> &fifo_w_out) {
#pragma HLS INLINE OFF
  /* Variable Declaration */
                    ...
                    #pragma HLS PIPELINE II=1
                      {
                        ...
                        #pragma HLS UNROLL
                          local_cout[c11][c10][c12] = (local_cout[c11][c10][c12] + (local_cin[0][0][c13] * local_w[0][0][0][c13]));
                        }
                        {
                          w_t2 fifo_data;
                        ...
                        if (c3 == 1 && c7 == 3 && c8 == 2 && c9 == 2)
                          fifo_cout_drain_out.write(local_cout[c11][c10][c12]);
                        {
                          cin_t2 fifo_data;
                          union {unsigned int ui; float ut;} u1, u0;
...
/* Module Definition */
void cin_PE_dummy_in(int idx, int idy, hls::stream<cin_t2> &fifo_cin_in) {
  /* Variable Declaration */
  int p0 = idx, p1 = idy; // module id
  /* Variable Declaration */
    ...
    for (ap_uint<2> c11 = 0; c11 <= 1; c11 += 1) {
      ...
      #pragma HLS PIPELINE II=1
        // simd
        {
          ...
          int split_idx = (c12) % 4;
          out_data = local_cout[c11][c10][c12 / 4];
          for (ap_uint<3> n = 0; n < 4; n++) {
          #pragma HLS UNROLL
          ...
          out_data = (data_split[3], data_split[2], data_split[1], data_split[0]);          local_cout[c11][c10][c12 / 4] = out_data;
        }
      }
    }
  }
